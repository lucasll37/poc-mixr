...
> gravação `.acmi` são resolvidos por caminho relativo (`./src/poc/single-thread/...`,
> `./shared/data/...`).
...
( ClockStation                                       ← shared/xclock: Station + controle de tempo
   ...
   simulation: ( WorldModel
        ...
        players: {
           ...
           // bandit1 NÃO mora mais aqui -- é o processo src/poc/bandit-dis, recebido só
           // pela rede via 'networks:' acima. Ver CLAUDE.md, seção "src/poc/bandit-dis".
        } ) )
```
...
- **O intruso (`bandit1`) não é declarado em `players:`.** Ele mora no processo
  [`src/poc/bandit-dis`](../bandit-dis/), pilotado por joystick físico (ou pelo `Autopilot` nativo,
  ...
  **mesma** lista que `Simulation::getPlayers()` devolve, então o radar/UBF reagem ao `bandit1`
  ...
  framework, na seção "`src/poc/bandit-dis`" do [CLAUDE.md](../../CLAUDE.md).
...
./build/src/poc/single-thread/src/single-thread -deterministic 300 | grep "^frame=300 "

# o piso de terreno está vivo? (controle negativo, sem recompilar)
sed 's/terrainClearance: ( Meters 800 )/terrainClearance: ( Meters 0 )/' \
    src/poc/single-thread/configs/scenario.epp.in > /tmp/sem-piso.epp.in
./build/src/poc/single-thread/src/single-thread -deterministic 12000 | grep '^frame=' > /tmp/com.txt
./build/src/poc/single-thread/src/single-thread -deterministic 12000 -f /tmp/sem-piso.epp.in \
    | grep '^frame=' > /tmp/sem.txt
diff -q /tmp/com.txt /tmp/sem.txt      # DEVEM diferir

# o que o replay recebeu
grep -o "Name=[^,]*,Type=[^,]*,Color=[^,]*,CallSign=[^,]*" \
     src/poc/single-thread/data/recordings/mission.acmi | sort -u
...
./build/src/poc/single-thread/src/single-thread -threads 1 -deterministic 1000 | grep "^meta="
```
