...
// std::out_of_range (estoura o tipo) -- sem isto a excecao escapava ate o
// topo e virava um abort() sem nenhuma mensagem apontando QUAL argumento
// estava errado. Toda outra entrada invalida desta aplicacao (arquivo de
// cenario, player ausente, banco de elevacao corrompido -- ver TerrainData/
// StationBuilder/Fleet) sai por std::exit com uma mensagem clara; um '-threads
// abc' devia se comportar da mesma forma, nao travar com um sinal.
long parseLongOrDie(const std::string& flag, const std::string& token)
{
   try {
      return std::stol(token);
   } catch (const std::exception&) {
      std::cerr << "[main] " << flag << " espera um numero inteiro, recebi '"
                << token << "'" << std::endl;
