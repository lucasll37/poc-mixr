...
> `.acmi` são resolvidos por caminho relativo (`./src/poc/multi-thread/...`, `./shared/data/...`).
...
```
$ ./build/src/poc/single-thread/src/single-thread -threads 1 -deterministic 2000 | grep 'frame=2000 player=falcon1'
frame=2000 player=falcon1 n=... e=... alt=... elev=... agl=... bt=EVADE ...

$ ./build/src/poc/multi-thread/src/multi-thread   -threads 1 -deterministic 2000 | grep 'frame=2000 player=falcon1'
...
./build/src/poc/multi-thread/src/multi-thread -threads 1 -deterministic 500 \
  | grep 'frame=500 player=falcon1' | grep -o 'dec=[0-9]*'

# as duas pocs, lado a lado, em passo fixo (iguais até ~1e-6 m -- ver 9.2)
./build/src/poc/single-thread/src/single-thread -threads 1 -deterministic 2000 | grep 'frame=2000 '
./build/src/poc/multi-thread/src/multi-thread   -threads 1 -deterministic 2000 | grep 'frame=2000 '
```
