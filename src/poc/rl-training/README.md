...
mesmo motivo que separa `models/flight` (o modelo) de `src/poc/*` (quem
...
**Treina com `MlpPolicy`, nao `MultiInputPolicy`** -- `train.py` acha um
`FlattenedObservation` por cima de `MixrFlightEnv` que achata o
...
fica em `train.py`, no CONSUMIDOR -- o `Dict`/`Discrete` continua sendo o
contrato publico de `src/rl` para quem quiser usa-lo direto.

## Exportar para producao

`tools/export_onnx.py` converte um checkpoint treinado em `.onnx`, no
contrato que `bt/nodes/OnnxPolicyAction` (`models/flight`) espera -- ver o
