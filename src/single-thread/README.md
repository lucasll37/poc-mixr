...
> `.acmi` são resolvidos por caminho relativo (`./src/single-thread/...`).
...
cujo ciclo roda em `updateData()`. É daí que vem o nome do subprojeto: **uma** thread para os
quatro agentes, em sequência — a caixa de cima, o frame de tempo crítico, continua fatiada entre
as `numTcThreads` threads do pool. Manter a decisão dentro da fase 3 do tempo crítico exigiria um
...
( Station
   ...
   simulation: ( WorldModel
        ...
        terrain: ( SrtmHgtFile path/file: S23W043.hgt )  ← banco de elevação
        players: {
...
- **`terrain:` é slot do `WorldModel`, e a factory dele não vem de graça.**
  ...
  `mixr_factory.cpp`, o `( SrtmHgtFile )` não constrói nada e o mundo fica sem chão, em
  silêncio. Quem consulta o banco é o `Player::updateElevation()` nativo, e é de lá que saem
  os campos `elev=`/`agl=` da linha de status. Detalhes e armadilhas no `CLAUDE.md`.

**As altitudes do cenário não são arbitrárias.** Cada falcon voa no **pico do próprio circuito
de patrulha + 300 m** (falcon1 1750, falcon2 1850, falcon3 2050, falcon4 2100 m), porque com
terreno carregado o `CRASH_EVENT` nativo passa a valer: abaixo do solo o player vai para
`CRASHED` e **congela**. O intruso voa a 2400 m — acima de todos —, e é isso que faz o
desconflito vertical da evasão ser para **baixo**, que é o único caso em que o terreno tem algo
a dizer.
...
- **A histerese é do modelo, não da árvore.** `ContactDetected` não pergunta "estou vendo o
  intruso agora?" e sim `domain::ThreatPolicy::engaged()`, que continua verdadeiro por
  `evadeHold` segundos depois de a pista sumir. Sem isso os ramos 2 e 3 alternam para sempre —
  ver a caixa abaixo.

> **Ciclo-limite observado no Tacview — as aeronaves voavam "batendo asa".** Os ramos 2 e 3
> comandam sentidos opostos sobre o **mesmo** objeto: fugir do intruso e ir até o ponto avisado.
> A quebra de 110° tira o intruso do setor do radar (`searchVolume` = ±30° em azimute), a pista
> some no mesmo instante, o ramo de apoio assume e traz a aeronave de volta — que reaquisita e
> quebra de novo. Medido no modo determinístico: ±25° de banco (saturando o `maxBankAngle: 30`)
> com período de ~24 s, indefinidamente. Duas correções, ambas em `domain/ThreatPolicy`:
>
...
> | depois | 3 | 4 | 8 | 6 |
>
> As que sobram são manobras legítimas (apoio a um alerta, troca de perna da patrulha), cada uma
> uma curva única que termina nivelada. Um rótulo novo separa os dois estados no status:
> `EVADE` (quebrando com a pista na tela) e `BREAK` (terminando a quebra no arrasto da histerese).

**O piso anti-CFIT do desconflito vertical.** "Desconflitar para baixo" só faz sentido enquanto
houver espaço embaixo. A regra descia `evadeClimb` a partir da altitude corrente contra um piso
absoluto de 200 m — que sobre esta serra fica ~1500 m *dentro* da montanha. Com o terreno
carregado, o alvo passa por `domain::clampToTerrain()`: nunca abaixo de
`elevação + terrainClearance` (800 m), com o piso absoluto sobrando como rede para quando não há
elevação. Como o alvo é fixado uma vez, na entrada da manobra, o piso também é avaliado uma vez.

Medido em 12 000 frames, comparando `terrainClearance: ( Meters 800 )` com `( Meters 0 )` — o
controle negativo que o próprio slot permite, sem recompilar:
...
`maxClimbRateMps: 8.0` e `evadeHold: 30 s`, o C310 desce ~330 m por engajamento — então uma
folga "bonita" de 300–500 m recorta um alvo que a aeronave nunca alcançaria, e o dump sai
idêntico ao do controle negativo. O piso continua correto; só é invisível. Ver a seção de
terreno do `CLAUDE.md`.
...
```bash
./build/src/single-thread/src/single-thread -deterministic 2000   # passo fixo, sem thread periódica
./build/src/single-thread/src/single-thread -threads 2            # força numTcThreads
```
...
>
> A **[poc/multi-thread](../multi-thread/)** é exatamente este subprojeto com essa única troca
> feita, para medir a diferença em vez de argumentar sobre ela. Resultado medido: em passo fixo
> as duas produzem estado **idêntico** (o lockstep já colocava a decisão no mesmo instante); em
> tempo real, a poc/multi-thread decide a 50 Hz em vez de 10 Hz e dispensa o artifício.
>
> O pool T/C é idêntico dos dois lados — `numTcThreads` e `-threads N` funcionam igual lá. O que
> os dois nomes separam é a thread da **decisão**, não a da simulação.
...
`processTimeCriticalTasks()`. `make check-single-thread` e `make check-multi-thread` continuam
valendo.
...
congelado. Nada se move; a decisão apenas não para. Vale para o `SimAgent` da poc/single-thread;
o `FlightAgentTC` da poc/multi-thread só é atingido no seu caminho de background.
