...
> são resolvidos por caminho relativo (`./src/multi-thread/...`).

**Por que ela existe.** A seção 8 do README da poc/single-thread termina numa ressalva: lá o determinismo é
...
> A linha `histerese da evasão` existe porque sem ela as duas pocs voavam "batendo asa": os ramos
> `ContactDetected → ReportAndEvade` e `AlertReceived → SupportAlert` comandam sentidos opostos
> sobre o mesmo intruso e alternavam indefinidamente. O diagnóstico e a correção estão na seção 6
> do README da poc/single-thread — e valem igual aqui, porque o modelo é o mesmo: **a taxa de decisão não
> tinha nada a ver com o problema** (1, 2 ou 4 threads, 10 Hz ou 50 Hz, os números eram os mesmos).

`make compare-single-multi` mostra o tamanho real da mudança:

```
Only in src/multi-thread/include/xnative: FlightAgentTC.hpp     ← a classe nova (87 linhas, 24 de código)
Only in src/multi-thread/src/xnative:     FlightAgentTC.cpp     ← 92 linhas, 43 de código
...
> **Sobre o namespace:** continua `mixr::xnative`, igual ao da poc/single-thread, de propósito — é o que
...
`behavior` (é a armadilha 10.7 do README da poc/single-thread), então não há nada a propagar.
...
Compare com a poc/single-thread: lá a caixa ★ ficava na segunda coluna, junto com o gravador.
...
O conteúdo do agente (estado, árbitro, votos, os ~16 parâmetros do `BtBehavior`) é **idêntico**,
linha por linha. Muda a classe, o lugar na árvore e o fato de não haver mais `actorPlayerName`.

Todo o resto do `.epp` também é igual, incluindo o banco de elevação — a mesma referência
(Serra do Mar, lat -22.25 / lon -42.48), o mesmo `terrain: ( SrtmHgtFile ... )` no `WorldModel`
e as mesmas altitudes derivadas do pico de cada circuito. Ver a seção 4 do
[README da poc/single-thread](../single-thread/README.md#4-a-árvore-de-objetos-do-cenário) e a
seção de terreno do `CLAUDE.md`.

**Uma diferença de comportamento que o terreno expõe:** o `SimAgent` da poc/single-thread decide
em `updateData()`, no mesmo laço de background em que o `Player::updateElevation()` atualiza a
elevação; o `FlightAgentTC` daqui decide na fase 3 do frame, a 50 Hz contra um background de
10 Hz, então lê uma elevação de até 100 ms atrás (~8 m percorridos). Irrelevante para um piso
com centenas de metros de folga, e sem efeito no determinismo — em `-deterministic` o laço faz
`tcFrame()` e `updateData()` em sequência no mesmo passo.
...
### 6.1 O que a poc/single-thread tinha de ressalva
...
`make check-multi-thread` roda 2000 frames com **1, 2 e 4 threads T/C** (mais uma repetição de 4) e
...
Isso funciona pelas mesmas três razões da poc/single-thread, que continuam valendo palavra por palavra:
...
`AlertDatalink` faz, e é o que sustenta o `make check-multi-thread`:
...
### 7.7 O caminho desta poc, fim a fim — e o que a poc/multi-thread muda nele

É o mecanismo pedido pelo enunciado: um evento de um player que muda o comportamento dos outros.
A peça é **a mesma** da poc/single-thread (`AlertDatalink` é byte a byte idêntico nas duas). O que muda é
...
falcon3 detecta bandit1 (radar nativo -- 7.3)
   └─ FASE 3: FlightAgentTC::controller()                  ← na poc/single-thread isto era updateData()
...
| | poc/single-thread | poc/multi-thread |
|---|---|---|
...
Ou seja: as três decisões de modelagem abaixo já estavam certas na poc/single-thread, mas duas delas ainda
não estavam sendo **exercidas**. Aqui passam a ser o que sustenta o `make check-multi-thread`.
...
1. **O handler roda na thread do EMISSOR.** O `sendMessage()` nativo chama `event()` direto no
   ...
   corrente. *(Já valia na poc/single-thread: lá o emissor estava no background e o receptor podia estar na
...
2. **A entrada não é fila FIFO.** A fusão é **comutativa** — vence o contato de menor distância;
   ...
   *(É esta que muda de estatuto: na poc/single-thread, com um único emissor serial, a ordem era estável de
...
> passa a transmitir ~50 alertas/s onde a poc/single-thread transmitia ~10 — números em
...
> período mínimo entre transmissões no nó da árvore —, não o canal. (O `ReportAndEvade` já
> transmite apenas enquanto há pista viva: no arrasto da histerese, rótulo `BREAK`, ele cala.)
...
```
$ ./build/src/single-thread/src/single-thread -threads 1 -deterministic 300 | grep 'frame=300 player=falcon1'
frame=300 player=falcon1 n=9255.883978869 e=458.464915958 alt=3000.003703427 hdg=92.740379131 ... sent=199 recv=0

$ ./build/src/multi-thread/src/multi-thread       -threads 1 -deterministic 300 | grep 'frame=300 player=falcon1'
...
passo fixo da poc/single-thread, `updateData()` roda imediatamente depois do `tcFrame()` e com o mesmo `dt` —
...
poc/multi-thread continua garantindo.
...
`ReportAndEvade` transmite um `TacticalAlert` por decisão, então em tempo real a poc/multi-thread emite
~50 alertas/s onde a poc/single-thread emitia ~10. Nada quebra (a fusão é comutativa e o `holdTime` do
...
./build/src/single-thread/src/single-thread -threads 1 -deterministic 300 | grep 'frame=300 '
./build/src/multi-thread/src/multi-thread         -threads 1 -deterministic 300 | grep 'frame=300 '
```
...
Igual à poc/single-thread, e pelo mesmo código (`shared/xclock/`, uma cópia só para as duas):
...
`fastForwardRate` — ao contrário do `SimAgent` da poc/single-thread, que decide no laço de background e
...
*antes* do teste de freeze: **[seção 14 do README da poc/single-thread](../single-thread/README.md#14-controle-de-tempo--acelerar-frear-pausar)**.
