...
# build convencional deste projeto (models/player/A4/build/tests/), mas o
# teste do meson passa o caminho de verdade explicitamente (nao pode
# presumir que o build dir se chama "build").
#
# uso: models/player/A4/tools/sync_tree_models.py [--check] [--binary PATH]
import argparse
import re
import subprocess
import sys
...
def main() -> None:
    ...
    if args.check:
        if outdated:
            sys.exit(
                "Desatualizados (rode models/player/A4/tools/sync_tree_models.py sem --check): "
                + ", ".join(outdated)
            )
