...
(o mesmo espírito de `A4`/`missile`, não `modelo1`/`modelo2`).
...
#    à raiz do repositório. Copiado direto para models/player/meu-modelo/,
...
5. **`xnative/factory.cpp`** → atualize as três listas (o `if/else`, `NOMES[]`, `METAS[]`) para
   ...
   eliminar `xnative/` e fazer como `models/player/missile/src/plugin.cpp`: a factory inline, sem
   indireção.
