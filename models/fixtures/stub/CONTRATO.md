...
uma disciplina não escrita — e foi escrever o `stub` que obrigou a descobrir a lista.

O `stub` é o teste desta página: um modelo de ~270 linhas, sem árvore de comportamento e sem
...
Os slots que o cenário usa estão em `models/fixtures/stub/src/stub.cpp` — as tabelas ali são a lista
...
O `stub` lê o `Gimbal` direto pela API do MIXR, em oito linhas, sem usar o `RadarScan` do
`flight`. É a prova de que a obrigação é **publicar**, e não usar um helper nosso.
...
`flight` instala o dele em `dist/share/mixr-plugins/flight/`. Um modelo que precise de
dados próprios instala junto e o cenário aponta para lá.

## 5. O que o contrato NÃO impõe

Nem pilha interna, nem árvore de comportamento, nem `domain/`. O `stub` não linka a
...
- Nada aqui é verificado pelo compilador. A verificação é o `stub` rodando; não há como o
