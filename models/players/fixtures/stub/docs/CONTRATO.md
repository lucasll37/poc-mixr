...
`mixr::models::x<nome-do-modelo>`, nunca solto. `models/players/A-4` (`domain::` solto no global) é
a exceção histórica, cara demais para corrigir agora — **não** é o exemplo a copiar.
`models/players/missile` e `models/players/template` já nascem certos; o raciocínio completo (por
