...
- **`RLBridgeBehavior` trivial** (`genAction()` devolve sempre `nullptr`) — sétimo nome de
  ...
  `RLBridgeBehavior` no `provides:` quando `models/players/A-4` passou a publicá-lo, e `provides:` é
...
- **O diretório pai passou de `models/player/` para `models/players/`** — este projeto passou a
  morar em `models/players/fixtures/stub/`. `git mv`, histórico preservado. Detalhe da varredura
...
- **`Makefile` autocontido** e **`README.md`** — as outras peças obrigatórias à época (o
  ...
  está (`ROOT := $(abspath ../../..)`, três níveis, por morar em `models/players/fixtures/`); um modelo
