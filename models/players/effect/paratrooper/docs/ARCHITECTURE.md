...
modelo real já aconteceu — `sandbox/C-130_paratrooper-6DOF` é um `models/players/air/C-130` liberando
...
`models/players/air/C-130` já usa hoje como placeholder (`C130ParatrooperPlaceholder`, também
...
2. **Compatibilidade de EDL com o mecanismo que já existe em C-130.** `xnative::
   ActionParatrooperRelease` (em `models/players/air/C-130`) já libera qualquer `AbstractWeapon*` cujo
   ...
   **só EDL** — a classe concreta declarada na estação + `provides:` — sem tocar C++ nenhum em
   ...
   Ver `models/players/air/C-130/docs/ARCHITECTURE.md`, seção "A liberação de paraquedista".
...
**Medido rodando**, num cenário de bancada (não versionado) com o C-130 de `models/players/air/C-130`
...
`xnative::ParatrooperAgentTC` é uma cópia estrutural de `models/players/air/C-130/src/xnative/
FlightAgentTC.cpp` (que por sua vez adapta `models/players/air/A-4`), resolvendo as mesmas três
