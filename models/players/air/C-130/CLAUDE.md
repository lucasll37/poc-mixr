# CLAUDE.md — models/players/air/C-130
...
## Descompasso motor/thruster no dado JSBSim vendorizado — corrigido

A primeira versão deste modelo copiou `t56.xml` (`<turbine_engine>`, calcula empuxo direto)
pareado com `t56_prop.xml` (um `<propeller>`, que espera potência/HP) — um descompasso de tipo
**no próprio dado JSBSim vendorizado**, não introduzido por este modelo. Sintoma: velocidade
...
(~160 s), isso terminava em **estol de verdade e colisão com o terreno** — não visível nos testes
curtos da primeira versão, só ao rodar `sandbox/C-130-6DOF` (rota mais longa) por mais tempo.

Corrigido: `data/jsbsim/engine/t56_prop.xml` foi removido e o thruster de cada um dos 4 motores em
`C130.xml` trocado para `direct` — mesmo padrão já comprovado em
`models/players/air/A-4/data/jsbsim/aircraft/A4/A4.xml` (`J52`, também `<turbine_engine>`, também
pareado com `<thruster file="direct">`). Se a velocidade voltar a não sustentar, a causa provável
é a mesma classe de bug (motor/thruster incompatível), não calibração de PID — ver o achado
completo no cabeçalho de `data/jsbsim/aircraft/C130/c130ap.xml`.
