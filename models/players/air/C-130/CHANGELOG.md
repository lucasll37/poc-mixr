...
- **`ParatrooperPlaceholder` nascia colado no C-130, e aparecia como bloco genérico no Tacview**
  ...
  `models/players/effect/paratrooper`, o placeholder herdava offset ZERO de `AbstractWeapon::dynamics()`
...
- **`src/poc/c130-airdrop` passou a liberar o `Paratrooper` real** (`models/players/effect/paratrooper`,
  ...
  `models/players/effect/paratrooper` reaproveita trocando só o EDL"). Mudança só de EDL: a classe
...
- Cenário novo, `sandbox/C-130_paratrooper-6DOF`: **a integração de verdade** com
  `models/players/effect/paratrooper` — um C-130 libera 30 `( Paratrooper )` reais (não mais o
...
- `C130ActionParatrooperRelease` + `C130ParatrooperPlaceholder`: a capacidade de liberar um
  ...
  `models/players/effect/paratrooper` reaproveita trocando só o EDL.
...
- **Corrigido, mesmo dia**: o par motor/thruster vendorizado originalmente copiado para
  ...
  `direct` (mesmo padrão já usado por `models/players/air/A-4`'s `J52`, também `<turbine_engine>`) —
