...
datalink — em comparação com `models/players/air/A-4` (o modelo de produção completo, com árvore de
...
- `C130.xml` — **cópia curada** do vendorizado (não o original em `shared/`), com duas edições:
  ...
  `models/players/air/A-4/data/jsbsim/aircraft/A4/A4.xml`, que tem a mesma edição pelo mesmo motivo).
- `c130ap.xml` (novo) — adaptado de `models/players/air/A-4/data/jsbsim/aircraft/{c310,A4}/*ap.xml`:
...
thruster para `direct` (mesmo padrão já comprovado em `models/players/air/A-4/data/jsbsim/aircraft/A4/
...
concreta de paraquedista** — é isso que já deixou a troca pelo `models/players/effect/paratrooper` real
...
`models/players/effect/paratrooper` — mirror do `dynamics()` de `xparatrooper::Paratrooper`, sem herdar
