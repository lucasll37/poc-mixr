...
2. **Libera um paraquedista** a partir de um `Steerpoint`, tratado como **liberação de ARMA**
   ...
   contra uma classe concreta. O modelo `models/players/effect/paratrooper` (produção, FSM completa:
...
`"FlightAgentTC"`/`"FlightState"`/... como `models/players/air/A-4` usa. `tests/guard/check_colisao_fabrica.py`
