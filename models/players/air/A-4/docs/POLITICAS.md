...
./build/app/src/app -file src/poc/dis/flight/configs/scenario.edl.in                # ou o comando de -deterministic abaixo
```

Quando a regra se provar, copie de volta para `models/players/air/A-4/configs/policy_example.py` — é essa a
...
PYTHONPATH=./dist/python src/poc/rl-training/.venv/bin/python3 src/poc/rl-training/tools/export_onnx.py \
    --sb3 src/poc/rl-training/runs/ppo_falcon1.zip -o models/players/air/A-4/configs/policy_example.onnx
```
...
./build/app/src/app -file src/poc/dis/flight/configs/scenario.edl.in -numTcThreads 4 -deterministic 600
```
...
| | |
|---|---|
| o script de exemplo | `models/players/air/A-4/configs/policy_example.py` |
| a política de exemplo | `models/players/air/A-4/configs/policy_example.onnx` (**pesos aleatórios**) |
| as árvores | `models/players/air/A-4/configs/flight_tree_{py,onnx}.xml` |
...
| a porta `schema` dos nós / resolução `"classic28"`\|`"all"`\|ad-hoc | `models/players/air/A-4/include/bt/ObservationSchema.hpp` |
| o registro de campos do A-4 (nome → como ler) | `models/players/air/A-4/include/domain/WorldViewFieldRegistry.hpp` |
