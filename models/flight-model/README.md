# `flight-model` — o modelo de produção
...
domain/     regras puras          libstdc++ só                    ← 42 testes, sem MIXR
   ↑
bt/         nós da árvore         BehaviorTree.CPP + domain/      ← 15 testes, sem MIXR
   ↑
...
| `libflight_model_tc.so` | `multi-thread` | `-DFLIGHT_MODEL_TC_AGENT` liga o `FlightAgentTC` |
...
```bash
make test-models        # domain (42) + tree (15) + native (9) = 66 casos, ~1 s
```
