...
const FLIGHT_SNIPPETS = {
  // AgentTC e' framework puro (mixr::base::ubf), nao modelo -- deveria vir do
  // extrator automatico (tools/extract_execution_chain.py), mas fica de fora
  // porque a classe nao e' dispatched por fabrica nenhuma neste fork (grep em
  // contexts/src/mixr/src/base/factory.cpp: "UbfAgentTC" nao aparece) -- o
  // mesmo criterio de concretude que ja deixa AbstractState/AbstractBehavior/
  // AbstractAction fora do MODEL global (ver comentario acima). Curada aqui a
  // mao, arquivo/linha reais (Agent.cpp:170), pelo mesmo motivo que
  // FlightAgentTC::controller abaixo e' curada: e' o metodo que a trilha "de
  // updateTC() ate o tick()" (Comportamento E step-by-step) precisa mostrar.
  "AgentTC::updateTC": {
  ...
  "FlightAgentTC::controller": {
    ...
    "lines": [
      ...
      "   // que thread do pool T/C este aviao decidiu. O core nao alcanca mais esta",
...
const MISSILE_SNIPPETS = {
  // Os quatro a seguir sao a DESCIDA -- de Station::updateTC() (topo do
  // quadro de tempo critico) ate o AgentTC::updateTC() que, la em
  // FLIGHT_SNIPPETS, ja repassa para FlightAgentTC::controller(). Nenhum e
  // curado sobre invencao: os dois de Simulation.cpp e o de Component.cpp
  // sao trechos REAIS fora da janela de 60 linhas que o extrator
  // (tools/extract_execution_chain.py) captura por metodo -- mesma razao
  // pela qual "Player::updateTC (AGL<0)", mais abaixo, tambem e curado a
  // mao em vez de reusar o SNIPPETS["Player::updateTC"] auto-extraido
  // (truncado ANTES do trecho que interessa aqui). Component::updateTC nem
  // aparece no SNIPPETS automatico -- mesmo motivo de AgentTC::updateTC em
  // FLIGHT_SNIPPETS: Component e classe abstrata, nunca dispatched por
  // fabrica, entao fica fora do criterio de concretude do extrator.
  "Simulation::updateTC (fase 3, despacho)": {
  ...
  "Player::updateTC (fase 3 -> BaseClass)": {
...
const MISSILE_TRACE = [
  ...
  },
  {
    ...
    stage: 0,
    cls: "Station",
    method: "updateTC(dt) -- o topo do quadro de tempo crítico",
    call: "chamado pela StationTcPeriodicThread nativa, a tcRate Hz -- o ponto de partida real desta trilha inteira",
    src: "Station::updateTC",
    hl: [15, 16],
    tag: null,
    flag: null,
    note: [
      "É AQUI que a trilha começa de verdade -- não no tick() do primeiro nó de árvore (ver o passo 'envelope', ainda a vários passos daqui), nem no controller() do agente (dois passos abaixo). Station::updateTC() roda numa thread NATIVA própria, criada por createTimeCriticalProcess() -- nenhum código deste modelo a chama; sete passos fixos (Timers, ioHandler, dispositivos de entrada) sempre rodam antes de qualquer player ser tocado.",
      "'if (sim != nullptr) sim->tcFrame(dt)' é a única linha que interessa a este míssil: tcFrame() é um invólucro fino (Component.cpp) que só embrulha estatística de tempo opcional em torno de 'this->updateTC(dt)' -- despacho virtual, que aqui cai em Simulation::updateTC() (próximo passo). Station não conhece 'a4_shooter' nem 'míssil' nenhum -- só agrega UMA Simulation.",
    ],
  },
  {
    id: "simulation-updatetc",
    stage: 0,
    cls: "Simulation",
    method: "updateTC(dt) -- o laço de 4 fases, uma vez por quadro",
    call: "setPhase(f) para f em 0..3, despachando cada fase à lista de players via updateTcPlayerList (próximo passo)",
    src: "Simulation::updateTC (fase 3, despacho)",
    hl: [7, 14],
    tag: null,
    flag: null,
    note: [
      "dt0/4.0 é o MESMO 'dt dividido por 4 na descida' que reaparece, multiplicado de volta, em FlightAgentTC::controller() (BaseClass::controller(dt * 4.0), dois passos adiante) -- o dt do FRAME inteiro sobrevive à viagem, só passa dt/4 pelas quatro fases porque cada fase é chamada uma vez por quadro, não uma vez por dt integral.",
      "reqTcThreads == 1 (mostrado aqui) é o ramo mais simples de ler; com numTcThreads > 0 o mesmo updateTcPlayerList() é despachado em paralelo, um por thread do pool (tcThreads[i]->start0(...)), com esta própria thread assumindo o último lote e uma barreira (SyncThread::waitForAllCompleted) fechando a fase antes da próxima começar -- é o que a poc flight/multi-thread mede como determinístico com 1, 2 e 4 threads.",
    ],
  },
  {
    id: "simulation-updatetcplayerlist",
    stage: 0,
    cls: "Simulation",
    method: "updateTcPlayerList() -- de qual thread o a4_shooter é atendido",
    call: "percorre a lista de players a cada n-ésimo item, a partir de idx -- ip->tcFrame(dt) é quem finalmente alcança o Player",
    src: "Simulation::updateTcPlayerList",
    hl: [16, 20],
    tag: null,
    flag: null,
    note: [
      "'every n'th player starting with the idx'th' é o round-robin que reparte a lista de players entre as threads do pool: com reqTcThreads==1 (idx=1, n=1) esta função varre a lista INTEIRA sozinha; com múltiplas threads, cada chamada (uma por thread, idx diferente) pega só a sua fatia -- é essa aritmética, não uma partição por nome, que decide em qual thread o a4_shooter é decidido a cada quadro.",
      "ip->tcFrame(dt) é o MESMO invólucro fino do passo anterior (Component.cpp), agora despachando para AbstractPlayer -- que por polimorfismo cai em Player::updateTC(dt0) (próximo passo), com o a4_shooter como 'ip'.",
    ],
  },
  {
    id: "player-updatetc",
    stage: 0,
    cls: "Player",
    method: "updateTC(dt0) -- fase 3 não faz nada sozinha; quem desce são os SUBCOMPONENTES",
    call: "switch(phase) cai em 'case 3: break;' -- o trabalho de fase 3 desta classe é vazio; BaseClass::updateTC(dt) é quem importa",
    src: "Player::updateTC (fase 3 -> BaseClass)",
    hl: [0, 13],
    tag: null,
    flag: null,
    note: [
      "O comentário nativo já entrega a resposta: 'our subsystems in the components list (e.g., pilot, nav, sms and obc) are updated by our call to BaseClass::updateTC()'. Player::updateTC() não sabe que o a4_shooter tem um agent: (FlightAgentTC) -- ele só repassa dt para TODOS os filhos, igual faria para pilot/nav/sms/obc.",
      "BaseClass aqui é Component (mixr::models::Player deriva de mixr::simulation::AbstractPlayer, que deriva de base::Component) -- BaseClass::updateTC(dt) é Component::updateTC(dt), o próximo passo, e é ele quem de fato acha o agent: dentro de components:.",
    ],
  },
  {
    id: "component-updatetc",
    stage: 0,
    cls: "Component",
    method: "updateTC(dt) -- a recursão GENÉRICA que alcança o agente",
    call: "percorre components: (getComponents()) e chama obj->tcFrame(dt) em CADA filho -- pilot, nav, sms, obc, e agent",
    src: "Component::updateTC",
    hl: [10, 15],
    tag: null,
    flag: null,
    note: [
      "Nenhuma menção a 'BtBehavior', 'FlightAgentTC' ou 'agent:' em lugar nenhum deste método -- é código 100% genérico de mixr::base::Component, o mesmo que qualquer Component com filhos usa. O EDL do a4_shooter declara agent: ( FlightAgentTC ... ) como mais um item de components:; é essa lista que o laço percorre.",
      "obj->tcFrame(dt) despacha virtualmente -- para o filho agent, isso cai em AgentTC::updateTC(dt) (próximo passo, já em FLIGHT_SNIPPETS: framework puro, não específico de míssil). Para pilot/nav/sms/obc, cairia em Autopilot::updateTC/Navigation::updateTC/etc. -- fora desta trilha.",
    ],
  },
  {
    id: "agenttc-updatetc",
    stage: 0,
    cls: "AgentTC",
    method: "updateTC(dt) -- o último degrau antes do controller()",
    call: "controller(dt) -- sem filtro de fase nenhum aqui; quem filtra é FlightAgentTC::controller() (próximo passo)",
    src: "AgentTC::updateTC",
    hl: [2, 2],
    tag: null,
    flag: null,
    note: [
      "mixr::base::ubf::AgentTC é framework NATIVO (Agent.hpp/.cpp), não deste modelo -- 3 linhas, sem lógica própria. A recursão de Component::updateTC() (passo anterior) chama isto em TODA fase (0..3), 4x por quadro -- é FlightAgentTC::controller(), o próximo passo, quem filtra 'world->phase() != 3' e ignora as outras três chamadas.",
      "'de onde tudo começa' (rótulo do próximo passo) segue certo no sentido de DECISÃO -- é ali que a fase certa é reconhecida e o dt do frame inteiro é reconstruído. Mas em termos de CHAMADA, o começo de verdade é Station::updateTC(), seis passos atrás nesta trilha.",
    ],
  },
  {
    ...
    stage: 0,
    cls: "FlightAgentTC",
    method: "controller(dt) -- onde a decisão passa a reconhecer a fase",
    ...
    flag: null,
    note: [
      "Os seis passos anteriores (Station → Simulation → Player → Component → AgentTC) rodam para QUALQUER Component do a4_shooter, não só para este agente -- é aqui que a cadeia deixa de ser genérica: world->phase() != 3 filtra as outras três chamadas de AgentTC::updateTC() por quadro (dynamics/transmit/receive), e só a de fase 3 segue adiante.",
      "BaseClass::controller(dt * 4.0) é quem desce para base::ubf::Agent::controller() (próximo passo) -- FlightAgentTC não decide nada sozinho, só publica no xboard (thread, contagem), reconstrói o dt do quadro inteiro (o mesmo dt/4 que desceu em Simulation::updateTC(), lá atrás, multiplicado de volta) e repassa pro framework UBF genérico.",
    ],
  },
  {
    id: "agent-controller",
    ...
    <>
      <div className="mx-body" style={{ paddingBottom: 110 }}>
        <p style={{ fontSize: 12.5, color: "var(--muted)", maxWidth: 900, marginBottom: 12 }}>
          Do <b style={{ color: "var(--ink)" }}>Station::updateTC()</b> -- o topo do quadro de tempo crítico -- até o <b style={{ color: "var(--ink)" }}>event(KILL_EVENT, ...)</b> do
          outro lado, passando pelo <b style={{ color: "var(--ink)" }}>tick()</b> que decide disparar no meio do caminho -- cada passo é código de verdade (modelo A-4/míssil
          ou fonte nativo do MIXR, <code className="mx-mono">contexts/src/mixr/</code>), na ordem em que roda, com o instante exato em que um evento nativo é (ou não é)
          emitido. Cenário de referência: <code className="mx-mono">sandbox/A4-6DOF-MISSILE</code>.
        </p>
