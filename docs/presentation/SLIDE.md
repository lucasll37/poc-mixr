### I. A proposta — uma reestruturação para devolver produtividade e escala

[slide 1]
Simulação aeroespacial é um domínio desafiador por natureza. É uma dificuldade  **inerente ao domínio** . Estamos falando de muitos agentes — aeronaves, sensores, armas, sistemas de navegação — cada um com uma dinâmica própria, e que precisam reagir e causar efeitos uns nos outros. 

[slide 2]
E aqui está o ponto que eu quero deixar bem claro: quando alguém implementa um modelo novo, o trabalho não termina em implementar a dinâmica desse modelo. O trabalho real é descrever o comportamento que ele tem frente ao pre-existentes modelos e eventos — e vice-versa. Um míssil novo precisa saber como reagir a um chaff que já existia. Uma aeronave nova precisa ser vista corretamente por um sistema de alerta que foi escrito antes dela existir.

[INCLUIR PLACEHOLDER PARA FIGURA]
...
[slide 5]
Foi com esse diagnóstico que o time de framework desenvolveu um metaprojeto, que é uma prova de conceito que propõe uma reestruturação de como a base da nossa aplicação se organiza hoje. A expectativa não é remover a dificuldade de modelar — como eu disse, isso não é possível — é recuperar a produtividade e a capacidade de escalar o desenvolvimento de modelos novos, atacando tudo que é atrito evitável. São várias frentes, e eu vou passar por cada uma.

[slide 6]
**Uso idiomático das ferramentas.** MIXR e BehaviorTree.CPP não são bibliotecas genéricas, são frameworks com opiniões fortes sobre como o código deveria se organizar. A aposta aqui é usá-los da forma como foram desenhados para ser usados, em vez de reconstruir por fora aquilo que eles já resolvem por dentro. Isso é o que faz a dinâmica de eventos que eu mencionei realmente funcionar a nosso favor.

[slide 7]
**Organização de código.** Até pouco tempo atrás, o que hoje é um projeto só estava espalhado em repositórios diferentes — `asa-models`, `asa-libs`, `asa-models-r`, o próprio MIXR, cada um com seu próprio ciclo, sua própria forma de versionar, sua própria distância dos outros. Na proposta, isso está tudo em um único repositório integrado. Não é só conveniência: é a diferença entre ter o contexto, a grande big-picture da aplicação de ponta a ponta.

[INCLUIR PLACEHOLDER PARA FIGURA]


[slide 8]
**Uma esteira de CI forte.** Testes automatizados em várias camadas — da regra de domínio mais isolada até o binário rodando de verdade — e diretivas de Makefile que deixam claro, no dia a dia, o que fazer para configurar, compilar, testar e rodar um modelo. 

[INCLUIR PLACEHOLDER PARA VIDEO]


[slide 9]
**Um contrato forte para o desenvolvimento de modelos.** Todo modelo, para entrar nesse repositório, carrega consigo a mesma espinha dorsal: documentação de arquitetura em `docs/`, e um `CHANGELOG.md` que registra o que mudou e quando
...
[slide 11]
[INCLUIR PLACEHOLDER PARA VIDEO DO APP]

[slide 12]
[INCLUIR PLACEHOLDER PARA VIDEO DA DOCUMENTAÇÃO]

[slide 13]
[INCLUIR PLACEHOLDER PARA VIDEO DO EDL-BUILDER]

[slide 14]
**Padronização de ambiente.** Extensões utilitárias do VS Code que fazem o dia a dia menos artesanal: realce de sintaxe para os arquivos de cenário — e, mais importante, uma validação de verdade desses arquivos, que roda o mesmo parser que o framework usa em produção, antes mesmo de tentar rodar a simulação.

[INCLUIR PLACEHOLDER PARA FIGURA]


[slide 15]
E, olhando para frente: essa base é pensada para ser **integrável com o asa-engine** — o conjunto que transforma o binário de simulação num serviço de verdade, com node, manager, handler, e que ofere endpoints para o `asapy` e à `webstation`. E, num segundo momento, com o ambiente virtual (Flight Gear).

[USE O DIAGRAMA ATUAL]
...
[slide 16]
Reescrever modelos que já existem é, admitidamente, um desafio grande — e um desafio que já sabíamos, desde o início, que teria de ser encarado. E esse desafio vem com um benefício concreto: reescrever é a oportunidade de recuperar um conhecimento que hoje só existe dentro do próprio código — e código é, por natureza, uma forma difícil de guardar conhecimento. E esse é, sinceramente, o melhor momento possível para fazer esse trabalho. Temos hoje ferramentas de IA que tornam a tarefa de ler um código antigo, entender sua intenção, e reescrevê-lo com documentação e testes junto, muito mais viável do que era há pouco tempo.

[slide 17]
O primeiro passo dessa reestruturação é exatamente este metaprojeto **open-source** , com todo o esqueleto do que pode vir a ser, no futuro, uma refatoração completa da ASA, com provas de conceito que mostram até onde essa estrutura pode ir (uso de OONX, scripts python, RL, etc) de forma nativa, adotando boas práticas de desenvolvimento.
...
É simples: seguir o `TOUR.md`. É um roteiro guiado. Ele leva você a subir o ambiente, rodar um cenário de verdade, ver a simulação acontecendo no Tacview, explorar o painel de controle, abrir o manual interativo. E, para quem quiser ir mais fundo, uma segunda trilha: o editor visual de cenários, o Groot, a decisão em Python, o ambiente de treino de aprendizado por reforço. NO PRIMEIRO CONTATO, NÃO USE IA. NÃO FAÇA AS PRESSAS, DE QUALQUER JEITO. TIRE UM TEMPO PARA FAZÉ-LO. OS INSGHTS SERÃO MAIS RICOS. ME DIVERTI FAZENDO ISSO, ESPERO QUE GOSTEM DE TESTAR A FERRAMENTA.

[INCLUIR PLACEHOLDER PARA FIGURA]

[slide 23]
O convite não é só "leia a documentação". É:  **estresse o metaprojeto** . Rode, quebre, proponha melhorias, reporte erros. Ele foi feito com a filosofia de ser amigável, plug-and-play — mas isso não significa que ele esteja livre de erros. Significa que encontrar esses erros, e reportá-los, já é parte esperada e valiosa do processo.
...
A expectativa é que cada pessoa que passar por esse tour deixe esse framework com a cara do que gostaria de trabalhar no futuro. Que a gente consiga, com o tempo, chegar a um núcleo o mais próximo possível do imutável — uma base estável o suficiente para que o esforço de cada equipe vá para o que é específico.



[slide 25]
Não pontuei, mas talvez o ganho mais silencioso de tudo isso: gerar um forte contexto técnico comuns à todos os desenvolvedores, não um "só entendo meu quadrado".

[slide 26]
Simulação aeroespacial vai continuar sendo desafiador — isso eu disse no começo, e repito agora, porque é verdade e vai continuar sendo. Mas a forma como escrevemos, testamos, documentamos e conversamos sobre os modelos que simulam esse domínio — essa parte, sim, é otimizável.
