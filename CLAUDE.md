...
frame, thread de **tempo crítico**: os 4 falcons decidem **em paralelo**, um por thread do pool,
a 50 Hz. `numTcThreads` distribui os players pelo pool de threads de tempo crítico do framework,
e a poc passa nos checks de determinismo com 1, 2 e 4 threads — nenhuma parte da simulação roda
numa thread só.

> **Não existe mais par de subprojetos gêmeos contrastando onde a decisão roda.** Havia antes
> `( SimAgent )` — nativo, componente da **`Station`**, decidindo em sequência no laço de
> background (`updateData()`, 10 Hz) — contra `( FlightAgentTC )`. `( SimAgent )` continua
> existindo como classe nativa do MIXR; só não é usado por nenhuma poc deste repositório.

Há um **segundo subprojeto** no mesmo grupo, `src/poc/dis/bandit/` (era `src/poc/bandit-dis/`;
o sufixo `-dis` saiu do nome quando a pasta `dis/` passou a dizer a mesma coisa), de natureza
diferente de `flight`: não tem agente UBF nenhum, é só o `bandit1` (o intruso que `flight`
persegue) rodando sozinho, num processo à parte, pilotado por joystick ou por `Autopilot` de
fallback, emitindo seu estado via **DIS nativo do MIXR** (`mixr::dis`) para quem quiser
recebê-lo — hoje, `flight`, que não tem mais um `bandit1:` local e o recebe só pela rede. Ver a
seção própria mais abaixo.

E um **terceiro**, `src/poc/python-flight/`, que muda um eixo diferente: não *onde* a decisão
roda — ele é a `flight` inteira, com o mesmo `( FlightAgentTC )` na fase 3 — e sim **em que
...
E um **quarto**, `src/poc/onnx-policy/`, que muda o mesmo eixo do terceiro e leva ao extremo:
quem decide é uma **rede neural**. Também é a `flight` inteira; a árvore de comportamento dela,
...
roda só a suíte do CORE; a do modelo é `make test-models`, separada; ver "Testes automatizados"
mais abaixo. `make models` sozinho é o alvo pra mexer só no modelo, sem tocar o core.

**Armadilha do `--reconfigure`, confirmada rodando — o primeiro `configure` depois de um
`clean`.** Os dois alvos `configure` deste repositório (o do core, `Makefile`, e o de cada
...
de um `.so` de plugin, sem `Station` nenhuma) — e `./src/node` (seção própria mais abaixo) é um
QUARTO binário, peer de `./app` mas deliberadamente independente dele, sem TUI.
...
dumps `frame=` — todos devem ser idênticos. Vale para as **três** pocs que decidem via
`( FlightAgentTC )` no pool de tempo crítico (`flight`/`python-flight`/`onnx-policy`). O mesmo
...
do core/modelo (nenhum `requires()` do `conanfile.py` da raiz o cita, e o core nunca linka contra
...
**Outros alvos do Makefile, fora do fluxo core/modelo acima**: `make open-docs` (a
...
**Atenção (corrigido — achado por auditoria, revisão completa do repositório):** `contexts/src/`
inteiro **é versionado** no próprio repositório — não é git-ignored, ao contrário do que este
parágrafo afirmava antes (`.gitignore` não tem entrada nenhuma para `contexts`; `git ls-files
contexts/` lista 975 arquivos rastreados). `CONTRIBUTING.md` já descrevia isso corretamente
("vendorizado e versionado no próprio repositório"). Por estar sempre presente num clone comum,
os geradores de `docs/manual/` (`tools/generate_manual_catalog.py`, `extract_class_diagram.py`,
`extract_execution_chain.py`) leem esse caminho **direto, sem nenhum fallback** para os headers
do Conan — o "se a pasta não existir, caia nos headers instalados pelo Conan" abaixo é a
orientação para uma pessoa investigando manualmente, não um caminho que o código de fato
implementa: `~/.conan2/p/b/mixr*/p/include/mixr/...`, `~/.conan2/p/b/mixr*/p/include/
DataRecord.pb.h` (o `.pb.h` gerado fica na **raiz** do include, não em `mixr/recorder/`) e
...
**Uma poc não tem código.** Desde que o `./app` virou o **runner único**, cada pasta sob
`src/poc/` é só dado:
...
`onnx-policy` e `built-in_mixr_1` rodados pelo `./app` saiu **byte-idêntico** ao
dos binários próprios que existiam antes (600 frames, `-threads 2`).

**A aplicação — `./app`, um arquivo por questão**, no namespace `app`:
...
`main.cpp` não implementa comportamento. `src/poc/dis/flight/configs/scenario.edl.in` é a
referência completa do padrão de cenário.
...
das configs (`flight`/`bandit`) declara o slot `host:`, então o `TacviewOutput` já sobe no default `"0.0.0.0"`:
o servidor já escuta em qualquer interface, o que falta é só o caminho de rede até a porta.

- **Linux nativo:** `hostname -I`/`ip addr show` no core dá o IP da LAN de verdade (não o
...
- **Binário dentro do WSL2 — tem um salto a mais.** `hostname -I` **dentro** do WSL2 devolve o IP
  ...
  LAN, mesmo com o Windows nela. É preciso encaminhar a porta no **Windows core**, como
  Administrador:
  ```powershell
...
`isFrozen()` (`Agent.cpp:59-62`) — um `( SimAgent )` decidindo em background continuaria avaliando
sobre um mundo estático com a simulação pausada (nada se move; a decisão só não para), mas
nenhuma poc deste repositório usa mais `( SimAgent )` (ver "Estrutura de um subprojeto" acima). O
`FlightAgentTC` da poc `flight` decide na fase 3, dentro do frame, então para junto.

`-deterministic` **não é afetado**: chama `station->tcFrame(dt)` direto, sem passar por
`processTimeCriticalTasks()`. `tests/determinism/check_determinism.sh` sobre `flight` segue
valendo.
...
**Desde que o `bandit1` virou o processo `src/poc/dis/bandit` (ver a seção própria mais abaixo), é lá
que o `ioHandler:` mora** — `flight` não declara mais nenhum. O cenário
...
`app/RealTimeRun.cpp` (de `bandit`) sonda o handler no mesmo lugar e na mesma taxa do
...
7. **Fallback gracioso para o `Autopilot`, adicionado quando o `bandit1` virou `src/poc/dis/bandit`
...
1. **`data/logs/` precisa existir no disco antes do `init()`** — `PrintHandler::openFile()` não
   ...
   não versiona em git.
2. **Mutex único em `Stream::~Stream()`** — a poc `flight` decide em paralelo, um
   ...
   precisava de mutex só para o console. Testado com um `treeFile:` inexistente:
...
3. **`-deterministic` desliga o log** (`xlog::setLoggingEnabled(false)`, chamado em `main.cpp`
   ...
   este). `tests/determinism/check_determinism.sh` sobre `flight` não é afetado.
4. **Substituiu `xnative::Log`** (`logLine(string)` + `setLoggingEnabled(bool)`, duplicado à mão
   entre as pocs gêmeas de então, só console, sem nível) — os 2 pontos de uso (`ubf/BtBehavior.cpp`,
   mensagem vazia/exceção ao carregar a árvore) migraram para `LOG(WARNING)`/`LOG(ERROR)`.
...
- **O motivo estrutural, e o que ele dá de graça:** `xlog` é `shared_library()` (a razão original
  ...
  buffer que o do core. **Confirmado rodando**: com `flight_tree.xml` removido do lugar, as 4
  linhas `LOG(ERROR) "[BtBehavior] falha ao carregar a arvore"` (uma por falcon, de dentro de
  `libflight.so`) aparecem na aba Log do `app` sem nenhuma ponte extra.
...
`models/events/README.md` para o porquê de não estar em `libs/`). Essas sete existem porque core e
...
threads T/C, porque essa ordem não é garantida neste framework (a poc `flight` decide em
...
`PATROL`). Medido: `tests/determinism/check_determinism.sh` com a semente ativa em
produção (`patrolMasterSeed: 20260903` nos 4 falcons de `flight`) passa
com 1, 2 e 4 threads T/C, dumps byte-idênticos.

**A prova de que é a SEMENTE que muda o resultado, não o agendamento entre threads, tem script
próprio**: `tests/determinism/check_patrol_seed.sh <binário> <rótulo> <poc> [frames]`, chamado com
`flight` como `<poc>`. Gera duas fixtures **hermético, modo `plain`** do
...
derrubando `plugin-modelo-estranho`/`plugin-deposito-terceiro`. Mesma classe de armadilha já
registrada para `RLBridgeBehavior` na seção `src/rl`: toda vez que `BtBehavior` de produção
ganha um slot novo que o `.edl` de produção usa, o mirror precisa aceitá-lo (mesmo que só para
ignorar), ou deixa de ser contrato-compatível com o cenário que os testes de plugin rodam
contra ele.

### `src/poc/dis/bandit` — o `bandit1` num processo próprio, emitindo DIS nativo do MIXR

Segundo subprojeto do grupo, de natureza diferente de `flight`: não é uma pilha nova nem um
agente novo, é **onde o `bandit1` mora agora** — antes um player local na poc de decisão, hoje
um processo à parte, pilotado por joystick físico (`libs/xjoystick`, com fallback pro `Autopilot`
scripted — armadilha 7 da seção `xjoystick` acima) e **emitido via DIS nativo do MIXR**
(`mixr::dis` — namespace real da lib, apesar do caminho do header ser `mixr/interop/dis/`) para
quem quiser recebê-lo. `flight` não tem mais um `bandit1:` local: recebe esse player **só pela
rede**, via `networks:`, exatamente como qualquer outra prova de interoperabilidade DIS de
verdade — duas instâncias separadas, não um truque de processo único.

**É bidirecional.** O `bandit` também **recebe** falcon1..4 (que `flight` emite de volta) — o
próprio Tacview do `bandit` (porta 1235) mostra as quatro falcons, não só o `bandit1`. Cada
`DisNetIO` só precisa dos dois blocos que fazem sentido pro seu lado: `outputEntityTypes:` pra
publicar os players locais, `inputEntityTypes:` pra materializar os da rede — os dois usam o
**mesmo** `disEntityType`, porque `bandit1` e `falcon1..4` são a mesma classe/`type:`
(`Aircraft`/`"C310"`) e o casamento de entrada é só pelo código numérico do fio, não pela string
— não há ambiguidade porque cada `DisNetIO` só ouve tráfego de quem só emite UMA coisa (`bandit`
só emite `bandit1`; `flight` só emite `falcon1..4`).
...
joystick — o `Autopilot` de fallback mantém `hdg=225` scripted) + `flight`
→ as falcons produzem `pista=bandit1@13.6NM`, `bt=EVADE`/`alerta<-falcon1(bandit1)`
se propagando, `bt=SUPPORT`, depois `bt=BREAK` — a cadeia UBF/BehaviorTree inteira reagindo a uma
aeronave que não existe em processo nenhum além do `bandit`. **E na volta**: a gravação
`.acmi` do `bandit` mostra os cinco `CallSign=` (`bandit1` + `falcon1..4`) — confirmando que
...
`bandit` — não precisou de ajuste nenhum além de existir.

**Esquema de portas** (mesma receita do exemplo do próprio MIXR — `MIXR-PATTERN-CONTEXT.md`
§6.7 — estendida para 2 processos no mesmo host): todo mundo **escuta** em `3000`; cada processo
...
`bandit`: `3001`. `flight`: `3002`. Os dois `DisNetIO`
...
2. **`initNetwork()` inicializa `netInput`/`netOutput` incondicionalmente** — os dois `netInput:`/
   ...
   (confirmado rodando: sem os dois presentes, a inicialização falha). Aqui os dois processos
...
3. **Processamento de rede roda dentro de `updateData()`, não do frame de tempo crítico** —
   ...
   background, **nenhuma chamada nova foi necessária** — nem em `bandit`, nem em `flight`.
...
4. **`mixr::dis::factory` não é encadeada por nenhuma outra factory nativa** — mesma armadilha
   ...
   `mixr_factory.cpp` de cada poc (as duas, `flight` e `bandit`), `networks: ( DisNetIO
...
5. **Comentários em `.edl`/`.edl.in` têm que ser ASCII puro.** Descoberto quebrando: um único
   ...
   requisito do parser. Confirmado: `scenario.edl.in` de `flight` é ASCII puro
...
6. **`applyCruiseThrottle` tem que ser replicado aqui** — o mesmo problema documentado em
   `app/Fleet.hpp` de `flight` (o autopilot do c310 fecha malha de rumo/altitude mas não de
...
Terceiro subprojeto de `src/poc/`. É a `flight` **inteira** — mesmo cenário, mesma pilha
nativa, mesmo `libflight.so`, mesmo `( FlightAgentTC )` decidindo na fase 3 — com **uma**
...
recompilar. Portas próprias (Tacview **1237**, DIS **3004**), então roda ao lado das outras duas.
...
7. **`tests/scenario/make_fixture.py` desviava a porta do Tacview com um `replace("port: 1234", ...)`
   ...
   o bloco `networks:`); a substituição de `flight` continua idêntica, byte a byte.
...
Quarto subprojeto de `src/poc/`. É a `flight` **inteira** — mesmo cenário, mesma pilha
nativa, mesmo `libflight.so`, mesmo `( FlightAgentTC )` decidindo na fase 3 — com **uma**
...
a byte; hoje não há core por poc — ver "Estrutura de um subprojeto").
...
`check_core_opaco.sh` (lista fixa envelhece em silêncio): `check_duplication.sh` — desde então
...
fases do frame de tempo crítico. Na `flight`, que decide na fase 3 a 50 Hz contra um
...
mesmo passo, e `tests/determinism/check_determinism.sh` sobre `flight` passa com 1, 2 e 4 threads T/C.
...
| `scenario` | o binario de verdade, com fixture, afirmando comportamento sobre as linhas `frame=` | 2 modos × 2 pocs |
...
> em copia nenhuma. Uma guarda sem nada para comparar ou falha (era o caso: o `find` voltava vazio
...
1. **O modo `-deterministic` NAO e hermetico com o cenario de producao.** O bloco `networks:` abre
   a porta DIS 3000 e ingere PDUs de quem estiver na LAN: com um `bandit` de outra sessao no
   ar, duas execucoes identicas divergem e `tests/determinism/check_determinism.sh` acusa falso
   nao-determinismo (medido: `frame=600 falcon1` deu `PATROL` com 1 thread e `SUPPORT` com 2 —
   porque o intruso da rede apareceu em uma e nao na outra). Por isso **todas** as fixtures de
   teste removem `networks:`, e os `check_determinism.sh` passaram a rodar em cenario hermetico.
   Com o cenario hermetico `flight` passa com 1, 2 e 4 threads em 2000 frames.
2. **As fixtures sao DERIVADAS do cenario de producao** (`tests/scenario/make_fixture.py`), nao
   ...
   processo so, ja que o intruso hoje mora em `src/poc/dis/bandit` e chega apenas por DIS.
...
5. **O ASan acusa 896 bytes em 22 alocacoes que NAO sao do modelo** — todas de partida, dentro do
   ...
   suite `memory`.
6. **`dec=` vem de `mixr::xboard::Readout`, não de um contador local do agente.** `dec=`/`thr=`
   sao incrementados no MESMO ponto de atuacao (`ubf::FlightAction::execute()`) — `app/StatusReport.cpp`
   e `app/DeterministicDump.cpp` leem esse `Readout` (ver `src/poc/dis/flight/README.md` §6.6.5).
   `FlightAgentTC` ainda mantem os proprios atomicos (`decisions`/`lastThreadTag`), mas nada mais
   os le — sao redundantes, nao a fonte do dump. A assercao **nao** e `dec == frames`: `flight`
   decide uma vez a mais na inicializacao (601 em 600 frames, identico nas 3 configuracoes de
   thread) — o warm-up `tcFrame()` que `app/StationBuilder.cpp` dispara logo apos o
   `RESET_EVENT` roda a fase 3 (e portanto o agente) uma vez antes do laco de frames comecar. O
   que se afirma e que `dec` avanca na **mesma taxa** que `frame` entre dumps consecutivos — mede
   a propriedade certa e ignora o offset de partida.
...
9. **Onde um script de teste ESCREVE nao pode sair de aritmetica de caminho sobre o binario.**
   `check_determinism.sh`/`check_patrol_seed.sh` montavam a pasta de saida como
   `$(dirname "$BIN")/../../../<rotulo>-...`, o que so caia num lugar sensato enquanto o binario
   estava a exatamente tres niveis (`build/src/poc/<poc>/src/<poc>`). Quando o `./app` virou o
   runner unico (`build/app/src/app`, um nivel a menos), os tres `..` passaram a aterrissar na
   **raiz do repositorio**, e cada `make check-*` largava uma pasta de lixo la -- nao versionada,
   mas suja e facil de commitar por engano. Hoje os dois derivam de `RAIZ` e escrevem em
   `build/tests-determinism/<rotulo>` e `build/tests-patrol-seed/<rotulo>`, junto com
...
Enquanto o modelo era um alvo do core, o `files()` dele listava os 24 `.cpp` e o `meson setup` do
core exigia o fonte — o oposto do que se queria provar. A guarda
...
> modelo) e nos testes do core que afirmavam sobre o nome antigo (`tests/plugin/test_plugin_load.cpp`,
...
pre-requisito e o SDK que o core publica uma vez (`make configure && make sdk`, na raiz); dai em
diante cada modelo se basta, e `make install-core` (o unico alvo que escreve fora do `./dist`
...
`install-core`/`uninstall-core` (a lista de `.so` publicada e o diretório de dados são
...
referenciado em algum `meson.build`. Ao contrário de `build`/`test`/`install-core`, o script em si
...
`install-core` de cada um, depositam SO em `plugins/` (lib, flat) e
...
**Por que**: compilar um modelo nunca precisou saber onde o CORE guarda os artefatos dele --
...
"contaminava" o estado de instalacao do core, mesmo que ninguem fosse rodar nada. Com o deposito
intermediario, `make build` (so compila o core) nunca precisa dos modelos, e `make models` (so
compila os modelos) nunca precisa saber que o core existe -- so `make install` (e os alvos que
...
com a única diferença (o `FlightAgentTC`) atrás de um `#ifdef` — dois artefatos,
`libflight.so`/`libflight_tc.so`. Hoje nem o `#ifdef` sobra: `FlightAgentTC` é sempre compilado e
sempre registrado, um artefato só, `libflight.so`. O alvo `make compare-single-multi` foi removido
junto com a poc gêmea que ele comparava.
...
Publicado pelo projeto do core em `dist/`: o contrato (`xplugin/PluginAbi.hpp`, header-only) e as
...
modelo), `libxrlbridge` (o comando/observação entre o core de RL — `src/rl/bindings/` — e
...
espalhado e não escrito: os 9 nomes de fábrica **e os slots deles**, as classes base obrigatórias,
e — a mais fácil de esquecer — o dever de **escrever no `xboard`**.

O cenário de falha que motivou o conserto: um terceiro entrega um `.so` que responde pelos 9 nomes,
deriva das bases certas, mas nunca chama o `xboard`. O core sobe, o cenário parseia, os aviões voam
...
- **O mirror de contrato do `template/`** (`libtemplate_mirror.so`, `src/mirror.cpp`, ~400 linhas)
  ...
  papel de `models/players/fixtures/stub/`, removido — não faz parte do scaffold copiável do
  `template/` (apagado quando `make new-model` copia o diretório). O teste
  `plugin-modelo-estranho` roda o cenário de **produção** contra ele trocando **apenas** o
...
1. **A BehaviorTree.CPP é ESTÁTICA** (`.a`, 35 MB, 447 símbolos `T` globais). O core largou
...
5. **O core não tem aresta Meson até o `.so`** (ele vem de `dist/`, de outro projeto), então o
...
1. Criar `src/poc/<nome>/` — nome descritivo, sem prefixo numérico — com **só dado**:
   ...
   `src/poc/dis/`, as duas que trocam DIS entre si), a pasta nova só precisa morar dentro do grupo;
...
1. **`xmsg_dep`/`xjoystick_dep` linkadas e as duas factories encadeadas** — os cenários das pocs
   declaram `( MsgFeed )` e `( JoystickIoHandler )`, e sem isso o parser recusa com "nome de
   fábrica desconhecido".
2. **`-f <arquivo>`** ao lado do `-scenario <chave>` — é por ele que todo teste roda (fixture
   gerada por `make_fixture.py`).
3. **A FROTA virou campo do cenário** (`ScenarioEntry::fleet`), não uma lista fixa no `main.cpp`:
   o `bandit` tem UM player, e `falcon1..4` fixo abortaria nele (`collectFleet()` sai com erro
   quando um nome não existe — e é isso que se quer).
4. **O `ioHandler` passou a ser sondado no laço da TUI**, no mesmo lugar e taxa em que o laço de
   tempo real das pocs o sondava (10 Hz, fora do frame T/C) — é o joystick do `bandit`.
5. `MetaObjectReport` voltou a reportar `MsgFeed`/`MsgReport`, e `Options` recuperou o
   `parseLongOrDie`/`parseIntOrDie` que só existia do lado das pocs.

**Bug real encontrado NA TROCA, e é o tipo de coisa que só aparece assim:** os três cenários
próprios do `./app` são herméticos, então a TUI nunca tinha aberto uma `Station` com `networks:`.
Ao rodar as pocs de DIS, o app morria em segundos — `SIGSEGV` em `List::Item::getValue()` e, depois
do primeiro conserto, `terminate` com `ExpInvalidRefCount` de dentro da própria
`Station::processNetworkInputTasks()`. Causa: **`Station::getNetworks()` NÃO é pre-ref()'d** — ao
contrário de `getPlayers()`, que o cabeçalho do framework marca como tal, ele devolve o membro cru
(`Station.cpp:639-642`). Havia **três** `unref()` sobre ele (dois em `app/ComponentTreeQuery.cpp`,
um em `app/DashboardState.cpp`, este último a 10 Hz), cada um soltando uma referência nunca tomada;
em poucos segundos o contador chegava a zero e o `PairStream` era destruído embaixo de quem o
percorria. Travado por `scenario-app-quit-dis`.


**Duas decisões de arquitetura, e o "porquê" de cada uma:**

- **FTXUI é a primeira dependência nova do CORE** nesta série de mudanças (as anteriores só
...
0. **A porta do Tacview do `./app` estava 1234 no fragmento, não 1236.**
   ...
   incluem por `@include:@`), e ele trazia `port: 1234` — a porta de `flight` — contra os 1236 que
   ...
   dois testes de saída seguidos (`scenario-app-quit`, com `patrol`, e `scenario-app-quit-dis`, com
   `flight`) passaram a abrir a MESMA porta — um deles morreu com `SIGSEGV` no caso do
...
4. **`app/MetaObjectReport.cpp` copiado de `flight` não compila sem trim.** O original
...
5. **`bandit1` nunca aparece no dump `-deterministic`, nem em `intercept`/`intercept_missile`** —
   ...
   mesmo em toda poc deste repositório (nem o modo `intruder` de `flight`
   ...
   falhava.
6. **`SimpleStoresMgr` (o `StoresMgr` do `scenario_intercept_missile.edl.in`) tem nome de fábrica
   `"StoresMgr"`, não `"SimpleStoresMgr"`** — o header documenta isso (`Factory name: StoresMgr`)
   mas é fácil escrever o nome da classe C++ por engano no EDL; a classe abstrata `StoresMgr` não
   é construível via EDL nenhuma. `( StoresMgr ... )` no EDL, não `( SimpleStoresMgr ... )`.
...
- **A árvore de BT agora é visualizável de verdade, dentro do card de detalhe (Frota E Mapa) —
  ...
  `models/players/A-4/src/bt/nodes/ReportAndEvadeAction.cpp`/`ReturnToBaseAction.cpp`), e nenhum dos
...
- **Breakpoint de árvore de BT** — "marcar um estado da bt de um dado elemento e rodar a
  ...
  - **ATUALIZAÇÃO (passada posterior): o timeout automático foi REMOVIDO.** Confirmado lendo o
    código de hoje — `app/include/app/BreakpointController.hpp`/`.cpp` (a máquina de estados foi
    extraída pra uma classe própria, testável sem `Station`/FTXUI, ver
    `tests/app/test_breakpoint_controller.cpp`) não tem `kBreakpointTimeoutSimSec` nem qualquer
    comparação contra `simSecNow` além do próprio hit — `grep` no repositório inteiro não acha
    mais essa constante. O header da classe documenta a decisão explicitamente: "Sem timeout:
    uma vez armado, o breakpoint só sai do ar por HIT ou por cancelamento manual —
    'continuar acelerado até o fim' e 'a busca deve ir até chegar nesse nó ou a simulação
    encerrar' foram pedidos explícitos que descartam qualquer desarme automático por tempo."
    Ou seja: hoje, armar um breakpoint sobre uma tag que nunca é atingida deixa a simulação
    rodando em modo rápido **indefinidamente**, até cancelamento manual (`[x]`) ou o cenário
    acabar — não há mais um teto de 300s simulados protegendo contra isso.
...
- **`thread` sempre "-" não era so falta de destaque — era falta de DADO.** Investigado antes de
  ...
  `models/players/A-4/src/xnative/FlightAgentTC.cpp::controller()` (o agente do pool T/C, usado só
  ...
  `models/players/A-4/src/ubf/FlightAction.cpp::execute()` (onde `xboard::setBehaviorLabel()`/
  ...
  membro. Precisou rebuildar o PLUGIN (`make models`), não o core — o campo é do modelo.
...
- **A pergunta "todos os players estão na mesma thread, isso está certo?" tinha resposta
  ...
  - **Migração mecânica dos três `.edl.in`** (`app/configs/scenario_{patrol,intercept,
    ...
    mesma ordem/motivo já documentado na seção `src/poc/dis/multi-thread` deste arquivo (fase 3 roda
...
- **`models/plugins/` (novo)** — quarta entrada em `models/`, mas de natureza DIFERENTE das
  ...
  pelo MESMO mecanismo `( PluginModule file: "..." provides: {...} )` — nenhuma mudança no core
  ...
  - `README.md` da pasta aponta pro mesmo `models/players/fixtures/stub/docs/CONTRATO.md` que já
  ...
  - **ATUALIZAÇÃO (passada posterior, ver "Desacoplando `models` de `dist/`" em "O MODELO é um
    ...
    (`models/plugins/`, via `install-core` de cada um), e é `make install` (alvo `sync-plugins`)
...
- **`tests/plugin/run_thirdparty_deposit.py` (novo) + `plugin-deposito-terceiro` (registrado em
  `tests/meson.build`, suíte `plugin`)** — reusa o STUB (`models/players/fixtures/stub`) fazendo o papel
  ...
  que o core normalmente consome direto de `dist/lib/mixr-plugins/`. Isso isola exatamente a
...
exibidas ao vivo. O `app` só EXIBE; quem escreve é o MODELO (`models/players/A-4`), de dentro do `.so`
aberto por `dlopen`.**
...
- **Consequência de graça, e é a parte interessante:** `xlog` é `shared_library()`, então há uma
  cópia só no processo — o `LOG(...)` do MODELO (`models/players/A-4`, dentro do `.so` aberto por
...
- **Quem PRODUZ o log é o MODELO, não o app** (pedido explícito, na sequência da primeira versão
  ...
  acima). A instrumentação foi para `models/players/A-4/src/ubf/FlightAction.cpp` —
...
- **ATUALIZAÇÃO (passada posterior): as outras pocs ganharam a MESMA correção de ordem.** O
  risco era real e não teórico — `single-thread`/`multi-thread`/`bandit`/`python-flight`/
  ...
  do `./app`, comparada na época por `tests/guard/check_duplication.sh`; `bandit` usava a variante sem
...
**Vigésima primeira passada: na aba F6, `[Espaço]` passou a pausar a SIMULAÇÃO de verdade, `[n]`
virou um passo de UM frame real, e a cadeia de chamadas do frame — nome de função, argumento e
`dt` — virou um painel próprio.**

- **O problema era honesto e mensurável**: `[Espaço]` mexia só no relógio da *animação*; a
  simulação seguia correndo, e o cabeçalho (`t=`/`sim=`/`thr=`) não refletia nada do que se
  comandava na aba. Agora `[Espaço]` chama o MESMO `doTogglePause()` do botão/tecla global, e a
  animação virou **escrava** desse estado: `setComponentFlowPlaying(flow, !paused)` roda a cada
  redesenho no Renderer mais externo — não na ação de tecla —, então a pausa vinda de qualquer
  lugar (o botão da barra principal, `p`, um breakpoint de BT atingido) para o pulso junto.
  `toggleComponentFlowPlaying()` deixou de existir: dois relógios independentes era exatamente o
  defeito.
- **`[n]` é um passo de verdade: um `Station::tcFrame(dt)`, com `dt = 1 / tcRate`.** Pausa antes,
  se estiver rodando (o gesto de qualquer depurador; e sem isso haveria duas threads dentro do
  mesmo frame). O passo é executado em `simThread`, **nunca** na thread de desenho, e só quando
  `ClockStation::isPaused()` — é seguro porque `ClockStation::processTimeCriticalTasks()` retorna
  cedo quando pausado, então a thread T/C nativa está viva mas não chama `tcFrame()`. Vai **antes**
  do `station->updateData(dt)` da mesma iteração, para a passada de fundo já ver o estado novo e
  drenar o gravador. Medido sob pty: 10 toques levaram `sim=` de 4,3 s a 4,5 s — **exatamente
  10 × 0,020 s**.
- **`app/FrameCallChain.{hpp,cpp}` (novo, puro — sem MIXR, sem FTXUI)**: a cadeia de chamadas de
  cada fase, com `arquivo:linha` do fonte do framework em cada item e os **valores vivos** de `dt`
  (de `getTimeCriticalRate()`/`getFastForwardRate()`). Não é traçado ao vivo — é a estrutura
  estática do código do framework, lida de `contexts/src/mixr/src/`, preenchida com os números da
  execução. As três coisas que ela existe para tornar óbvias, todas conferidas no fonte antes de
  escritas:
  1. **O `dt` desce dividido por 4 e volta multiplicado por 4.** `updateTcPlayerList` passa
     `dt0/4` (`Simulation.cpp:595`), e dentro de `Player::updateTC` o `dt4 = dt * 4`
     (`Player.cpp:561`) devolve o `dt` do frame inteiro para os módulos que rodam uma vez a cada
     quatro fases.
...
- **A informação do passo mora DENTRO do desenho, não num painel de texto ao lado.** A primeira
  tentativa foi um painel de cadeia de chamadas de 16 linhas embaixo do canvas — e ele roubava ~19
  linhas justamente da árvore, que é a peça que se quer olhar. Desfeito na mesma passada; o que
  ficou:
  - **Faixa de fases** (`renderFramePhaseStrip`) — um diagrama de pipeline ocupando a MESMA linha
    única que antes era um texto esmaecido: `FRAME T/C 50.0 Hz dt=0.020000s │ tcFrame → 0 dynamics
    → 1 transmit / 2 receive → 3 process ⇒ FUNDO 10.0 Hz │ Agent::controller → updateData`. O
    bloco da fase corrente vem preenchido com a cor da fase; os demais, esmaecidos. A seta dupla
    `⇒` marca a fronteira entre as duas **threads**.
  - **O caminho da recursão ACESO no próprio canvas.** Toda aresta que faz parte da descida desta
    fase sai na cor da fase, contra o cinza das demais — é literalmente `Component::updateTC()`
    chamando `obj->tcFrame(dt)` em cada filho, que é a recursão que a árvore representa. Uma
    **onda** branca percorre esse caminho de cima para baixo, um nível por passo de animação,
    usando o MESMO contador de redesenhos de `tickComponentFlowAnimation()` (não há relógio novo).
  - **O rótulo da chamada no próprio nó** (`nodeCallLabel()`): quem participa da fase corrente
    ganha, logo abaixo do nome, `dynamics(0.020s)` / `tx+rx(0.020s)` / `process(0.020s)` /
    `updateData(0.100s)` em amarelo. É a resposta a "que função, com que argumento" colada no
    componente que a executa.
  - **O card de detalhe ganhou a seção "nesta fase"**: o que ESTE componente executa agora e o
    **caminho da recursão** que chega até ele, com nome, argumento e `arquivo:linha`. Vai no card
    porque ele já tinha espaço sobrando e porque a informação é por-componente, não global.
  - **Estado grande e inequívoco**: `‖ SIMULAÇÃO PAUSADA` em amarelo / `▷ SIMULAÇÃO RODANDO` em
    verde, ao lado da fase — e o rótulo do botão de passo carrega o `dt` real
    (`[n] Passo 0.0200s`).

**Armadilhas confirmadas — não redescobrir:**

1. **`p.paused ? "texto" : {}` não compila** num parâmetro `const std::string&` — o `{}` não tem
   tipo dedutível no ternário. Precisa de `std::string{}` dos dois lados.
2. **`const bool x{a == b}`** dentro de uma lista de inicialização com comparação encadeada
   confunde o parser em algumas formas; `const bool x = (a == b)` é o que se usou aqui.
3. **O passo manual não pode rodar na thread de desenho.** `stepFrameRequests` é um
   `std::atomic<int>` drenado por `exchange(0)` no laço de `simThread` — a thread que já é dona de
   `updateData()`.
4. **O caminho aceso tem de atravessar galho RETRAÍDO.** Com os players fechados (o estado
   inicial), os participantes da fase 3 não estão no layout e o caminho não acendia nada — bem na
   hora em que o usuário mais quer ver por onde a chamada desceria. Por isso
   `ComponentTreeNode::subtreePhaseMask`, um bitmask das fases presentes na subárvore INTEIRA
   (inclusive escondida), calculado de baixo para cima na descoberta; um galho retraído que
   esconde um participante sai com o **rótulo na cor da fase**, convidando a abrir.
5. **O caminho da recursão no card é mostrado pela CAUDA, não pela cabeça.** O card tem altura
   fixa (a mesma de F1/F2) e o começo da cadeia é sempre o mesmo preâmbulo genérico — cortar pelo
   fim escondia justamente a linha que interessa. Medido: com a cabeça, `case 3: process(dt4)`
   ficava de fora. O `arquivo:linha` só aparece na linha ativa; nas outras competia por largura
   com o nome da função e cortava os dois.

- **Testado**: `tests/app/test_frame_call_chain.cpp` (alvo `app-frame-call-chain`, suíte `domain`,
  11 testes, sem MIXR e sem FTXUI) — o `dt` que desce dividido e volta multiplicado, cada fase
  destacando a chamada que de fato executa (as fases 1 e 2 destacam **as duas**), as fases de fundo
  caindo na outra cadeia, `dt0` zerado quando pausado, o `fastForwardRate` aparecendo no laço, o
  caminho da descida sendo um subconjunto coerente da cadeia (mantém a ativa e os ancestrais,
  descarta os ramos não tomados, nunca salta profundidade), e o rótulo curto do nó (só quem
  participa da fase corrente ganha um; o de fundo usa o `dt` de fundo). `tests/app/test_component_flow_state.cpp` acompanhou a
  troca do toggle pelo setter, incluindo o caso que a motivou: chamar com o valor que já vale é
  no-op, senão o contador de redesenhos zeraria a cada quadro e a animação nunca avançaria.
  Verificado sob pty: `[Espaço]` leva o cabeçalho a `PAUSADO (1x)` e congela `sim=` em 4,3 s por
  3 s de parede (com `t=` correndo, que é o certo — é tempo de parede); 10 × `[n]` movem `sim=` de
...
- **`xnative::ThreadTagProbe`** (`models/players/A-4/{include,src}/xnative/
...
- **Custou uma 9ª classe na factory — e por isso um `provides:` a mais em TODO cenário que
  ...
  execução.
- **`models/players/fixtures/stub/src/stub.cpp` ganhou a MESMA classe, trivial** (só
...
- **`GuidedMissile` também não tem agente** (só `dynamicsModel` + guiagem própria em
  ...
  ele o mesmo `ThreadTagProbe`. Mas `GuidedMissile` mora em `models/players/missile`, um
  **plugin Meson separado** (`libmissile.so`) — não linka nem conhece
  `models/players/A-4` — então a classe teve de ser reescrita lá (não reaproveitada por
  `#include`), no mesmo molde do resto do repositório (`models/players/fixtures/stub` já duplica
...
- **A MESMA investigação expôs um segundo problema, mais sutil, ainda não medido em
  ...
  PRIVADO de `models/players/A-4`** — cada `.so` que o tivesse duplicado teria seu
  ...
  `models/players/A-4/include/xnative/ThreadTag.hpp` para `libs/xboard/Board.hpp`
...
- **O teste que existia para `xnative::threadTag()`** (`ThreadTag.EstavelNaMesmaThreadEDistintoEntreThreads`,
  ...
  (suíte `domain`, nível core) — não porque o teste tenha mudado de forma, mas porque a
...
- **Varredura de documentação**: todo `README.md` sob `src/poc/**`/`sandbox/**` que invocava
  ...
  `.claude/rules/{core-app-src,models-plugin}.md`, `scripts/models.sh` e comentários do `Makefile`/
  `src/poc/meson.build`. **Não tocado, de propósito**: `docs/presentation/index.html` — NA ÉPOCA
  desta passada, slide deck órfão, sem alvo `make` que o gerasse ou abrisse (ver a correção
  abaixo) —, e as passadas anteriores desta seção, que são diário e descrevem o que era verdade
  NA ÉPOCA, não o estado atual.

  > **Correção (achado por auditoria de documentação, não uma nova passada de trabalho):** o
  > Makefile já tem `open-presentation: ## [TEMPORARIO] Abre docs/presentation/index.html (slide
  > deck).` — o deck deixou de ser inteiramente órfão. O alvo só **abre**, não regenera
  > (`index.html` continua escrito à mão e versionado, ao contrário do de `docs/manual/`), e o
  > próprio comentário do alvo já marca a situação como temporária ("remover quando a
  > apresentação saída de uso").
...
Quarto binário do core (depois de `app`, `edlcheck`, `plugininfo`), mas de natureza diferente dos
três: **super enxuto**, roda **uma simulação passada por argumento** (`./dist/bin/node
<arquivo.edl|.edl.in>`, mesma convenção de `edlcheck <arquivo>`), em tempo real, sem TUI — na
tela só aparecem linhas de log (`LOG(...)` de `libs/xlog`, já ligado no console por padrão; nada
de status line por frame). `Ctrl+C`/`SIGTERM` encerram de forma limpa.

**Decisão deliberada: `src/node` não reaproveita NADA de `./app/`** — nem `app/src/
mixr_factory.cpp`, nem `StationBuilder`/`Shutdown`/`TerrainData`/`ScenarioTemplate`
(`app/include/app/*.hpp` + `app/src/app/*.cpp`). Usa só as libs de framework compartilhadas de
verdade (`mixr`, `libs/xlog`, `libs/xclock`, `libs/xtacview`, `libs/xplugin`, `libs/xjoystick`,
`libs/xmsg`) como qualquer outro consumidor legítimo delas. Isso já tem precedente funcionando
neste repositório: **`src/rl/bindings/`** também constrói e roda uma `Station` a partir de um
`.edl` sem depender de `app/` — tem sua própria `mixr_factory.cpp`/`.hpp` e seu próprio
`StationBuilder.cpp`/`.hpp`. `src/node` segue a MESMA receita (par de funções `factoryBuiltin`/
`factory`, mesmo desembrulho do `base::Pair` que `edl_parser` devolve, mesmo `RESET_EVENT` +
frame de aquecimento), com duas diferenças exigidas pelo escopo mais amplo:

- **cadeia de fábrica COMPLETA** (`xplugin → xtacview → xclock → xjoystick → xmsg → simulation →
  models → terrain → dis → linkage → recorder → base`, em `src/node/factory.cpp`) — ao contrário
  de `src/rl/bindings`, que roda sempre o MESMO cenário hermético conhecido, `node` precisa rodar
  **qualquer** cenário de `src/poc/**`, que pode declarar terreno, DIS, joystick físico ou
  `MsgFeed`;
- **laço de tempo real de verdade** (`src/node/Run.cpp`) — `station->createTimeCriticalProcess()`
  uma vez, depois um laço a 10 Hz chamando só `station->updateData(dt)` com pacing ancorado no
  relógio de parede (mesma receita do extinto `RealTimeRun.cpp`/do laço de fundo de `./app`,
  redescoberta a partir dos headers do framework, não copiada de `app/`). Usa `( ClockStation )`
  de `libs/xclock`, que `src/rl` dispensa (lá a simulação avança passo a passo sob comando do
  Python).

**O preço aceito por essa independência**: `node` tem sua própria reimplementação pequena da
ordem de encerramento correta — `quiesceTimeCritical()`/`shutdownStation()` em `Run.cpp`, a
MESMA lógica documentada em `app/include/app/Shutdown.hpp` para um bug real do framework nativo
(thread de tempo crítico + fila do gravador sem teto podem travar o processo se
`SHUTDOWN_EVENT` disparar antes de calar a produtora) — reescrita aqui porque a causa é do
framework nativo, não de `app/`, então a correção precisa existir nos dois lugares
independentemente. `quiesceTimeCritical()` chama diretamente a API pública de `ClockStation`
(`requestTcStop()`/`waitForTcQuiesced(timeoutSec)`), com fallback nullptr-safe
(`setFastForwardRate(0)` + espera curta, sem prova positiva) se o cenário não usar
`( ClockStation )`. `shutdownStation()` tem o mesmo watchdog destacado que força `_Exit(0)` se o
teardown nativo travar além de `watchdogSec`.

**Único token de template suportado: `@NUM_TC_THREADS@`** — confirmado por varredura de TODOS os
`.edl.in` de produção sob `src/poc/**`: nenhum usa `@include:@` nem `@RUN_ID@` (só aparecem em
`app/`/`sandbox/`). Por isso `generateScenario()` (em `main.cpp`) é só uma troca de string, sem a
máquina genérica de `ScenarioTemplate.cpp`.

**O que fica de fora, deliberadamente, em relação a `./app`**: sem frota/`applyCruiseThrottle`
(não há conceito de "frota" — `node` não presume nenhum tipo de player), sem picker interativo,
sem `-deterministic`, sem sondagem de joystick nem push de identidade/varredura de radar do
Tacview (cosméticos do painel Mapa da TUI — o Tacview padrão via `DataRecorder` continua
...
**Nota de transparência**: `.claude/rules/core-app-src.md` cita "`./app` é o runner único das
pocs" como o invariante mais repetido deste repositório — mas essa regra é sobre
`src/poc/<nome>/` (cada pasta de cenário não ter mais o próprio executável, regime já aposentado
— ver "Ao adicionar um subprojeto novo" mais abaixo). `src/node/` não é uma dessas pastas: é uma
ferramenta irmã de `src/poc`/`src/rl`, que recebe QUALQUER cenário por argumento, não dona de
cenário nenhum. Ainda assim, vale registrar: `node` reintroduz, para um propósito diferente
(execução não-interativa, só-log, desacoplada da TUI), uma segunda cadeia de fábrica core
completa — o preço aceito, por pedido explícito, para manter `node` independente dos internos de
`app/`.
...
**É essa a causa mais provável de "o Groot não reconhece a sintaxe."** Confirmado rodando os 4
`flight_tree*.xml` contra um parser XML estrito: 3 de 4 tinham o problema (só `flight_tree_nav.xml`
já era limpo) — corrigido nos 3 (`--` → `-`, só dentro do comentário, sem tocar a árvore em si).
...
MESMA `BT::BehaviorTreeFactory` que o modelo de verdade monta (`registerNodes()` +
`registerSdkNodes()`, com um `NodeContext{}` vazio — seguro, porque `registerBuilder<T>()` só
guarda um construtor, nunca instancia nó nenhum) e chama a função nativa do BT.CPP para isto,
...
script é o processo que mantém os 4 arquivos em dia — sem isso, o Groot mostra os nós antigos
certos e o nó novo com "This model has not been registered" na hora de carregar. Os 4 arquivos de
...
cada um dos 4 `flight_tree*.xml` de produção já com o bloco — nenhum nó fica "sem modelo
registrado". Rodei também os cenários que carregam essas árvores (`flight`, `python-flight`,
`onnx-policy`, `full-systems-nav`) em `-deterministic`: o bloco extra no XML não
...
`update_bt_models.py`, que descobre TODA árvore de `configs/` por CONTEÚDO (todo `.xml` com
`<BehaviorTree>` dentro, nunca uma lista de nomes fixa) e sincroniza o `<TreeNodesModel>` de cada
uma com o registro atual — se o bloco já existe, substitui por inteiro (cobre nó removido E nó
novo); se a árvore ainda não tem bloco nenhum, insere um antes do `</root>` de fechamento, em vez
de abortar com erro. O teste `tree-model-sync` (`meson test`, suíte `tree`) passou a chamar
...
abria, nada era logado, e o Groot só dizia "Was not able to connect". Hoje o **core** confere isso
...
"não casei" e "ninguém casou" são indistinguíveis de lá; só o core tem a lista inteira.
...
patrulha (seção `libs/xrandom` acima) — **só funciona quando o agente do UBF é um
`( FlightAgentTC )` aninhado DENTRO do player**, caso de `flight` (`BtBehavior` mora dentro do
`UbfArbiter`, que mora dentro do próprio `falconN` — `container()` sobe até o `Player` de
verdade). Com um `( SimAgent actorPlayerName: falconN )`, componente da **Station** — a ligação
com o player é por **nome**, nunca por `container()` — o mesmo `findContainerByType` devolveria
`nullptr` (medido rodando, print em `stderr`, fora do `xlog`, quando este repositório ainda tinha
uma poc decidindo em background via `( SimAgent )`; ver "Estrutura de um subprojeto" acima —
nenhuma poc usa mais `( SimAgent )` hoje).

Consequência não investigada mais a fundo, registrada por precaução: `configurePlans()` usa esse
mesmo `findContainerByType` para derivar a semente de jitter da patrulha — um `( SimAgent )`
receberia `""` e os 4 falcons acabariam com a mesma sequência de jitter entre si (o master seed
continua correto). Isso não quebraria `tests/determinism/check_patrol_seed.sh` porque o script só
verifica reprodutibilidade entre threads e divergência entre sementes de CENÁRIO — nenhuma
verificação cobre se os 4 players do MESMO cenário têm sequências distintas entre si. Não
corrigido, fora do escopo desta feature, e hoje sem poc nenhuma para expor o caso.
...
core escreve `Command`/lê `Observation`, o modelo faz o inverso. **Sem chave por `playerId`** — v1
...
`provides:` (agora maior) do cenário de produção que ele roda contra.
...
1. **`import mixr_gym` TEM DE vir antes de `numpy`/`gymnasium` no processo Python**, se o script
   ...
   `libs/xplugin/PluginRegistry.cpp::loadModule()`, ao carregar `libflight.so`) — reproduzido
   ...
   carrega perfeitamente fora do Python (`./dist/bin/app -f src/rl/configs/scenario_rl.edl
   -deterministic 1` não crasha nunca). Por isso `mixr_gym/__init__.py` importa `._native` (sob `RTLD_GLOBAL`) ANTES
...
três; `make build` compila `_native*.so` como parte do core; `make install` deposita
...
- **`docs/manual/index.html`** (`make open-docs`, único alvo — sem `make docs` separado desde que
  ...
  **Correção (achado por auditoria):** a aba "Referência" não existe mais em `doc.jsx` hoje —
  ...
  3. **step-by-step** (nome em inglês por pedido explícito, mesma exceção já registrada para
     ...
     **ATUALIZAÇÃO (a trilha cresceu de 35 para 41 passos): o prólogo deixou de começar em
     `FlightAgentTC::controller()`.** A própria página dizia "Do `tick()` que decide disparar..."
     no texto de abertura, mas o primeiro passo de código já era `FlightAgentTC::controller()` --
     rotulado, então, como "o ponto de entrada real de TODA a cadeia desta trilha". Não era: seis
     chamadas REAIS acontecem antes dele, a cada quadro de tempo crítico, e nenhuma delas é
     específica de míssil nenhum -- são a mesma descida que a aba Simulação já anima (`Station` →
     `Simulation` → `Player` → `Component`), só que a trilha do míssil nunca a citava por nome.
     Seis passos novos, todos stage 0 ("contexto"), inseridos entre `configurePlans()` (que roda
     uma vez, no `reset()`) e o antigo primeiro passo de frame: `Station::updateTC` (reaproveita o
     `SNIPPETS` auto-extraído, só troca o `hl` para `sim->tcFrame(dt)`), `Simulation::updateTC` e
     `Simulation::updateTcPlayerList` (dois `MISSILE_SNIPPETS` NOVOS, curados a mão -- o laço de 4
     fases e o despacho round-robin por thread ficam FORA da janela de 60 linhas que o extrator
     captura por método, então o `SNIPPETS["Simulation::updateTC"]` auto-gerado, truncado, não
     serve), `Player::updateTC` (outro `MISSILE_SNIPPETS` novo, mesmo motivo -- o `case 3: break;`
     que cai para `BaseClass::updateTC(dt)` fica depois do corte de 60 linhas do extrator; o
     comentário nativo já entrega a resposta: "our subsystems in the components list ... are
     updated by our call to BaseClass::updateTC()"), `Component::updateTC` (a recursão GENÉRICA
     que acha o `agent:` dentro de `components:` -- nunca aparece no `SNIPPETS` automático porque
     `Component` é classe abstrata, nunca dispatched por fábrica nenhuma) e `AgentTC::updateTC`
     (framework puro, `Agent.hpp`/`.cpp` -- MESMO gap de concretude do passo anterior).

     **Achado ao preencher o último destes seis: `AgentTC::updateTC` já era referenciado pela aba
     Comportamento (`traceFlightPatrol`/`Evade`/`Python`/`Onnx`, o primeiro passo de cada uma,
     rótulo "AgentTC::updateTC(dt) → controller(dt)") sem NENHUM código exibido -- `flightSnip()`
     caía em `FLIGHT_SNIPPETS[key] || SNIPPETS[key]`, e a chave não existia em nenhum dos dois. O
     card renderizava só título/corpo, sem o bloco `.mx-code`, um "buraco mudo" que ninguém tinha
     notado porque nada quebrava -- só faltava conteúdo.** Corrigido na FONTE do problema, não no
     sintoma: `AgentTC::updateTC` (4 linhas reais, `Agent.cpp:170`, `controller(dt);`) entrou como
     entrada curada em `FLIGHT_SNIPPETS` -- não em `MISSILE_SNIPPETS`, porque `missileSnip()` já
     cai em `FLIGHT_SNIPPETS` como segundo degrau da cadeia de fallback (`MISSILE_SNIPPETS[key] ||
     FLIGHT_SNIPPETS[key] || SNIPPETS[key]`), então uma correção só ali já resolve os DOIS lugares
     de uma vez -- a aba Comportamento passou a mostrar código real neste passo também, de graça,
     sem editar `traceFlightPatrol()`/`Evade`/`Python`/`Onnx` nem `FlightDecision()`.

     O texto de abertura da aba ("Do `tick()` que decide disparar...") também mudou, para não
     mentir sobre onde a trilha realmente começa: agora fala de `Station::updateTC()` primeiro,
     `tick()` "no meio do caminho". Verificado sob jsdom (mesmo método já usado nesta
     seção, sem Chromium disponível): 41 passos navegam sem erro de console, contagem por estágio
     bate (`contexto` 7→13, os outros seis inalterados, soma 41), os quatro passos novos com
     snippet CUSTOM mostram `kind: "excerto -- ver arquivo completo"` e o `hl` certo (`sim->tcFrame
     (dt)` em `Station::updateTC`, o laço `for (f<4)` + despacho single-thread em
     `Simulation::updateTC`, o `if (count==index)` + `ip->tcFrame(dt)` em `updateTcPlayerList`, o
     `case 3` + `BaseClass::updateTC(dt)` em `Player::updateTC`), e os dois passos que reaproveitam
     `SNIPPETS`/`FLIGHT_SNIPPETS` sem alteração de dado (`Station::updateTC`, `AgentTC::updateTC`)
     saem com `kind: "C++"` (não truncado) como esperado.
...
versionado (não é hermético — depende de `contexts/src/mixr/` estar presente; essa árvore **é**
versionada no próprio repositório, não git-ignored como uma versão anterior deste parágrafo
dizia, e por isso está sempre presente num clone comum — os três geradores citados acima leem
esse caminho direto e **não têm** fallback pra headers do Conan, ao contrário do que a nota da
seção "Onde consultar o framework" chegou a sugerir antes de ser corrigida) e `open-docs` continua
...
lib-nem-.so>` (mesma convenção de `plugin:<nome>` que `models/players/<nome>/` já usa) — reaproveita
...
- A documentação de referência continua vivendo no `README.md` da raiz, no de cada
  ...
  escrever um MODELO novo (não mexer no core), o ponto de entrada é
...
- `build/` e `dist/` não são versionados (`.gitignore`); `build/` já foi destrackeado com
  `git rm -r --cached`. **Correção (achado por auditoria):** `contexts/src/` **é** versionado no
  próprio repositório — este item afirmava o contrário; ver a correção na seção "Onde consultar o
  framework" acima.
