...
(`models/players/air/A-4/src/bt/nodes/OnnxPolicyAction.cpp`) — sempre as três funções,
nesta ordem:
...
1. **`-Wl,--exclude-libs,ALL` é obrigatória.** `gnu_symbol_visibility: 'hidden'` **não** se aplica
   a objetos vindos de um `.a` — a mesma razão pela qual `models/players/air/A-4/meson.build` precisa dela
...
`models/players/air/A-4/meson.build` gera **quatro** artefatos do mesmo `model_sources`. Dentro do plugin
