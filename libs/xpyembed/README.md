# `libs/xpyembed` — Python embarcado dentro do frame
...
`src/poc/python-flight/configs/flight_tree_python.xml`:

```xml
<PyDecide script="./src/poc/python-flight/configs/policy/patrol.py"
          label="PY-PATROL"/>
```

**2. O script cumpre um contrato de uma função só** —
`src/poc/python-flight/configs/policy/patrol.py`:

```python
# def decide(obs) -> (heading_deg, altitude_m, speed_kts)
# obs: 28 floats na ordem canonica de libs/xrlbridge/ObservationFields.hpp
...
**3. O nó chama as três funções da API** — `PyDecideAction::tick()`, resumido:

```cpp
#include "xpyembed/PyEmbed.hpp"

if (!mixr::xpyembed::isAvailable()) {
   // sem interpretador -- FAILURE, o Fallback da arvore assume (ex.: ( Patrol ) nativo)
} else {
   scriptId_ = mixr::xpyembed::loadScript(caminho.value());   // preguicoso, 1x por caminho
}

std::array<double, XRLBRIDGE_OBSERVATION_SIZE> entrada{ /* a WorldView, achatada */ };
std::array<double, XRLBRIDGE_ACTION_SIZE>      saida{};

if (mixr::xpyembed::decide(scriptId_, instanciaId_,
                            entrada.data(), entrada.size(),
                            saida.data(), saida.size())) {
...
`instanciaId_` é um id **por instância do nó** (um contador atômico, não o `ScriptId`) — é o
`playerId` que a assinatura de `decide()` pede, e é o que garante que `falcon1..4`, todos rodando
o MESMO `patrol.py`, recebam cada um o seu próprio dicionário de globais (ver "Determinismo"
abaixo). `decide()` devolve `false` sem lançar/abortar em qualquer falha — script não carregado,
exceção no Python, retorno com forma errada —, e `PyDecideAction::tick()` traduz isso em
`BT::NodeStatus::FAILURE`, deixando o resto da árvore (`Sequence`/`Fallback`) decidir o que fazer.
...
| Latência por chamada, 4 threads | **17,6 µs** (o GIL serializa) |
...
`./tests/determinism/check_determinism.sh ./build/app/src/app python-flight 2000 python-flight` verde é cada `(script, chave)` ter o **seu próprio dicionário de globais**
...
1. **`gnu_symbol_visibility: 'hidden'` esconde a própria API.** Sem `XPYEMBED_API` nas declarações,
   ...
   duas vezes (aqui e em `libs/xinfer`), porque o sintoma só aparece se alguém for olhar.
