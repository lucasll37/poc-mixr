...
- Cada `models/player/<nome>/` é um projeto Meson **autocontido**: tem `Makefile`, `meson.build`,
  `tests/`, `docs/`, `README.md`, `CHANGELOG.md` próprios (guarda:
  `tests/guard/check_modelo_estrutura.sh`). `cd models/player/<nome> && make` configura/builda/
...
- **Nomes de fábrica MIXR são globais ao processo, não por-plugin.** Dois `.so` carregados juntos
  ...
  aconteceu de verdade (`ThreadTagProbe`, A4×missile — ver CLAUDE.md, "vigésima terceira passada").
...
- Modelo novo: `make new-model NAME=<nome> KIND=stub|template` (não escreve lógica nenhuma, só
  copia o esqueleto certo). Depois, siga `CONTRIBUTING.md` → `models/README.md` para registrar o
  cenário (`app/src/app/ScenarioCatalog.cpp`) e anote em `models/REGISTRO.md` (coordenação humana,
  sem enforcement automático).
- `models/player/fixtures/stub/` e `models/player/template/` **não são produção** — não entram na
