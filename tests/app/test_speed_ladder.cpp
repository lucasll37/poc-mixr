...
{
   auto normal{speedDisplay(false, false, 1.0, 0.0)};
   EXPECT_EQ(normal.label, "1x");
   EXPECT_EQ(normal.tone, SpeedTone::Green);

   EXPECT_EQ(speedDisplay(false, false, 4.0, 0.0).tone, SpeedTone::Yellow);
   EXPECT_EQ(speedDisplay(false, false, 0.5, 0.0).tone, SpeedTone::Cyan);
