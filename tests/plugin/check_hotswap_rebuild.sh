...
# layout que nao existe mais: models/players/A-4/Makefile builda em ./build dentro
...
publica_so() {
   cp -f models/players/A-4/dist/lib/mixr-plugins/libflight.so "$SO"
}
...
HDR=models/players/A-4/src/domain/PatrolPlan.cpp
BKP=$(mktemp)
cp "$HDR" "$BKP"
restaura() {
   cp "$BKP" "$HDR"
