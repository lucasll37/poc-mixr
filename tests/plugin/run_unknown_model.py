...
O que isso verifica, e nada mais verifica:
   que os 9 nomes de fabrica + os slots + as classes base + a escrita no
...
def main():
    ...
    ap.add_argument("--modelo", required=True, help="caminho do .so desconhecido a carregar")
    args = ap.parse_args()

    if not Path(args.modelo).is_file():
        print(f"FALHA: {args.modelo} nao existe -- rode 'make models'")
        return 1

    out = RAIZ / "build" / "tests-fixtures"
    out.mkdir(parents=True, exist_ok=True)

    base = out / f"{args.poc}-unknown-base.edl.in"
    subprocess.run(
    ...
    novo, n = re.subn(r'file:\s*"[^"]*"', f'file:     "{args.modelo}"', texto, count=1)
    if n != 1:
        print("FALHA: nao achei o 'file:' do ( PluginModule ) na fixture")
        return 1

    fixture = out / f"{args.poc}-unknown.edl.in"
    ...
    if not re.search(r"\[plugin\] carregado '[^']+'", saida):
        falhas.append("o modelo desconhecido nao foi carregado")
