#!/usr/bin/env python3
"""Lint LEVE de um arquivo .edl/.edl.in, contra o catalogo de --edl-catalog.
...
A verificação PROFUNDA e definitiva é o parser C++ de verdade, via
`make edl-check FILE=...` (app/src/edlcheck_main.cpp) -- este script existe
para pegar o erro mais comum ANTES de precisar compilar/instalar nada, e
para poder rodar como regressão contra os cenários reais do repositório
...
def load_catalog():
    if not CATALOG_PATH.exists():
        raise SystemExit(
            f"catalogo nao encontrado em {CATALOG_PATH} -- rode 'make edl-catalog' primeiro."
        )
...
def main():
    ...
    if args.json:
        ...
        if not result.errors and not result.warnings:
            print(f"OK -- {args.arquivo}: nenhum problema encontrado (lint leve; rode 'make edl-check' para o parser real).")
