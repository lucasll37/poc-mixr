...
| [A4-6DOF-MISSILE](A4-6DOF-MISSILE/README.md) | Dois A-4 (atirador + alvo): detecção por radar → envelope de disparo → guiagem proporcional → detonação de um míssil cinemático | A-4 + `missile` | independente — primeiro de uma série de exercícios (ver `TODO.md`) |
...
| [Navstar-3-constellation](Navstar-3-constellation/README.md) | Quatro satélites `Navstar-3`, mesma classe, quatro planos orbitais distintos (`raan` diferente em cada um) | `Navstar-3` (`models/others/Navstar-3`) | independente — único cenário orbital deste `sandbox/`, não pilota aeronave nenhuma |
